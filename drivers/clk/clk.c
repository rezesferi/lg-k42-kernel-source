...
#ifdef CONFIG_LGE_PM
#ifdef CONFIG_PROC_FS
static int clk_summary_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, clk_summary_show, PDE_DATA(inode));
}

static const struct file_operations clk_summary_proc_fops = {
	.open		= clk_summary_proc_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
...
#ifdef CONFIG_LGE_PM
#ifdef CONFIG_PROC_FS
	proc_create_data("clk_summary", 0444, NULL, &clk_summary_proc_fops, &all_lists);
#endif
#endif
