...
static void long_touch_handler(struct timer_list* arg)
{
	struct egistec_data *fp = from_timer(fp, arg, long_touch_timer);
...
#if ENABLE_TRANSLATED_LONG_TOUCH
		mod_timer(&fp->long_touch_timer, jiffies + (HZ * LONGTOUCH_INTERVAL / 1000));
#endif
		break;
...
#if ENABLE_TRANSLATED_LONG_TOUCH
	timer_setup(&fp->long_touch_timer, long_touch_handler, 0);
#endif
