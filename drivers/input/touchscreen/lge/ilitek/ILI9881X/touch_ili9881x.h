...
#ifndef LGE_TOUCH_ILI9881X_H
#define LGE_TOUCH_ILI9881X_H
...
#define ILI9881X_SLAVE_I2C_ADDR		0x41
#define ILI9881X_ICE_MODE_ADDR		0x181062
#define ILI9881X_PID_ADDR		0x4009C
#define ILI9881X_WDT_ADDR		0x5100C
#define ILI9881X_PC_ADDR		0x44008
#define ILI9881X_LATCH_ADDR		0x51010
#define ILI9881X_IC_RESET_ADDR		0x040050
#define ILI9881X_IC_RESET_KEY		0x00019881
#define ILI9881X_IC_RESET_GESTURE_ADDR	0x040054
#define ILI9881X_IC_RESET_GESTURE_KEY	0xF38A94EF
#define ILI9881X_IC_RESET_GESTURE_RUN	0xA67C9DFE
...
struct ili9881x_touch_abs_data {
	u8 y_high:4;
	u8 x_high:4;
	u8 x_low;
	u8 y_low;
...
/*
struct ili9881x_touch_shape_data {
	s8 degree;
	u8 width_major_high;
	u8 width_major_low;
	u8 width_minor_high;
...
*/
/* report packet */
struct ili9881x_touch_info {
	u8 packet_id;
	struct ili9881x_touch_abs_data abs_data[10];
	u8 p_sensor:4;
	u8 key:4;
	//struct ili9881x_touch_shape_data shape_data[10];
	u8 checksum;
} __packed;

struct ili9881x_debug_info {
...
struct ili9881x_gesture_info {
	u8 data[GESTURE_MODE_PACKET_INFO_LENGTH - 1];
	u8 checksum;
} __packed;

struct ili9881x_chip_info {
	u32 pid;
	u16 id;
	u8 type;
	u8 ver;
...
struct ili9881x_fw_extra_info {
	u8 build;
} __packed;
struct ili9881x_protocol_info {
	u8 command_id;
	u8 major;
	u8 mid;
	u8 minor;
...
struct ili9881x_ic_info {
	struct ili9881x_chip_info chip_info;
	struct ili9881x_fw_info fw_info;
	struct ili9881x_fw_extra_info fw_extra_info;
	struct ili9881x_protocol_info protocol_info;
	struct ili9881x_core_info core_info;
};

struct ili9881x_panel_info {
	u8 command_id;
	u8 nMinX;
	u8 nMinY;
	u8 nMaxX_Low;
...
#if 0
struct ili9881x_panel_extra_info {
...
struct ili9881x_tp_info {
	struct ili9881x_panel_info panel_info;
//	struct ili9881x_panel_extra_info panel_extra_info;
};

struct ili9881x_data {
	struct device *dev;
	struct kobject kobj;
...
//	struct delayed_work fb_notify_work;
	...
	struct ili9881x_ic_info ic_info;
	struct ili9881x_tp_info tp_info;
	struct ili9881x_touch_info touch_info;
	struct ili9881x_debug_info debug_info;
	struct ili9881x_gesture_info gesture_info;
...
extern void ili9881x_get_pc_latch(struct device *dev);
extern int ili9881x_dump_flash_data(struct device *dev, u32 start, u32 end);
extern void ili9881x_dump_iram_data(struct device *dev, u32 start, u32 end);
extern int ili9881x_switch_fw_mode(struct device *dev, u8 mode);
extern int ili9881x_ice_mode_disable(struct device *dev);
extern int ili9881x_ice_mode_enable(struct device *dev, int mcu_status);
extern int ili9881x_ice_reg_write(struct device *dev, u32 addr, u32 data, int size);
extern int ili9881x_ice_reg_read(struct device *dev, u32 addr, void* data, int size);
extern int ili9881x_reg_read(struct device *dev, u8 cmd, void *data, int size);
extern int ili9881x_reg_write(struct device *dev, u8 cmd, void *data, int size);
extern void ili9881x_get_trans_report(struct device *dev);
extern int ili9881x_tp_info(struct device *dev);
extern int ili9881x_reset_ctrl(struct device *dev, int ctrl);
extern int ili9881x_ic_info(struct device *dev);
extern int ili9881x_check_cdc_busy(struct device *dev, int conut, int delay);
extern void ili9881x_dump_packet(void *data, int type, int len, int row_len, const char *name);
extern u8 ili9881x_calc_data_checksum(void *pMsg, u32 nLength);
// int ili9881x_ic_info(struct device *dev);
// int ili9881x_tc_driving(struct device *dev, int mode);
// int ili9881x_irq_abs(struct device *dev);
// int ili9881x_irq_lpwg(struct device *dev);
// int ili9881x_irq_handler(struct device *dev);
// int ili9881x_check_status(struct device *dev);
// int ili9881x_debug_info(struct device *dev);

static inline struct ili9881x_data *to_ili9881x_data(struct device *dev)
{
	return (struct ili9881x_data *)touch_get_device(to_touch_core(dev));
}

static inline struct ili9881x_data *to_ili9881x_data_from_kobj(struct kobject *kobj)
{
	return (struct ili9881x_data *)container_of(kobj,
			struct ili9881x_data, kobj);
}
static inline int ili9881x_read_value(struct device *dev,
		u16 addr, u32 *value)
{
	return ili9881x_reg_read(dev, addr, value, sizeof(*value));
}

static inline int ili9881x_write_value(struct device *dev,
		u16 addr, u32 value)
{
	return ili9881x_reg_write(dev, addr, &value, sizeof(value));
}
...
extern int ili9881x_apk_init(struct device *dev);
extern int ili9881x_check_int_status(struct device *dev, bool high);
extern int ili7807_get_fw_data_wrapper(struct device *dev, u8 *wdata, int wsize, void *rdata, int rsize);
extern void ili9881x_get_ddi_reg_onepage(u8 page, u8 reg, u8 *data);

#endif /* LGE_TOUCH_ILI9881X_H */
