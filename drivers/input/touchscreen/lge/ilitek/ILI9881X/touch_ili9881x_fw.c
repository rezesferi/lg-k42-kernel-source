...
static int ili9881x_iram_read(struct device *dev, u8 *buf, u32 start, int len)
{
	int i, limit = 4*K;
...
//	u8 cmd[4] = {0};
	...
	for (i = 0, addr = start; addr < end; i += limit, addr += limit) {
		...
		if (ili9881x_ice_reg_read(dev, addr, buf + i, limit) < 0) {
...
void ili9881x_dump_iram_data(struct device *dev, u32 start, u32 end)
{
	struct file *f = NULL;
	mm_segment_t old_fs;
	loff_t pos = 0;
	...
	ret = ili9881x_ice_mode_enable(dev, MCU_STOP);
	if (ret < 0) {
	...
	ret = ili9881x_iram_read(dev, fw_buf, start, len);
	if (ret < 0)
		goto out;
...
static void ili9881x_flash_write_enable(struct device *dev)
{
	TOUCH_TRACE();

	if (ili9881x_ice_reg_write(dev, FLASH_BASED_ADDR, 0x0, 1) < 0)
		TOUCH_E("Pull CS low failed\n");

	if (ili9881x_ice_reg_write(dev, FLASH1_ADDR, 0x66aa55, 3) < 0)
		TOUCH_E("Write key failed\n");

	if (ili9881x_ice_reg_write(dev, FLASH2_ADDR, 0x6, 1) < 0)
		TOUCH_E("Write 0x6 failed\n");

	if (ili9881x_ice_reg_write(dev, FLASH_BASED_ADDR, 0x1, 1) < 0)
...
static int ili7808g_flash_poll_busy(struct device *dev, int timer)
{
	...
	if (ili9881x_ice_reg_write(dev, FLASH2_ADDR, cmd, 1) < 0)
		TOUCH_E("Write 0x5 cmd failed\n");

	do {
		if (ili9881x_ice_reg_write(dev, FLASH2_ADDR, 0xFF, 1) < 0)
			TOUCH_E("Write dummy failed\n");

		touch_msleep(1);

		if (ili9881x_ice_reg_read(dev, FLASH4_ADDR, &temp, sizeof(u8)) < 0)
...
static int ili9881x_read_flash_data(struct device *dev, u32 start, u32 end, u8 *data, int len)
{
	u32 i, index = 0;
	u32 tmp;
	...
	if (ili9881x_ice_reg_write(dev, FLASH2_ADDR, 0x03, 1) < 0)
		TOUCH_E("Write 0x3 failed\n");

	if (ili9881x_ice_reg_write(dev, FLASH2_ADDR, (start & 0xFF0000) >> 16, 1) < 0)
		TOUCH_E("Write address failed\n");

	if (ili9881x_ice_reg_write(dev, FLASH2_ADDR, (start & 0x00FF00) >> 8, 1) < 0)
		TOUCH_E("Write address failed\n");

	if (ili9881x_ice_reg_write(dev, FLASH2_ADDR, (start & 0x0000FF), 1) < 0)
		TOUCH_E("Write address failed\n");
	...
	for (i = start; i <= end; i++) {
		...
		if (ili9881x_ice_reg_read(dev, FLASH4_ADDR, &tmp, sizeof(u8)) < 0)
...
int ili9881x_dump_flash_data(struct device *dev, u32 start, u32 end)
{
	struct file *f = NULL;
	u8 *buf = NULL;
	mm_segment_t old_fs;
	...
	ret = ili9881x_read_flash_data(dev, start_addr, end_addr, buf, length);
	if (ret < 0)
		goto out;
...
static void ili9881x_flash_protect(struct device *dev, bool enable)
{
	TOUCH_TRACE();

	TOUCH_I("%s flash protection\n", enable ? "Enable" : "Disable");

	ili9881x_flash_write_enable(dev);

	ili9881x_ice_reg_write(dev, FLASH_BASED_ADDR, 0x0, 1);

	ili9881x_ice_reg_write(dev, FLASH1_ADDR, 0x66aa55, 3);

	ili9881x_ice_reg_write(dev, FLASH2_ADDR, 0x1, 1);

	ili9881x_ice_reg_write(dev, FLASH2_ADDR, 0x0, 1);
	...
	case 0xEF:
		if (flashtab->dev_id == 0x6012 || flashtab->dev_id == 0x6011) {
			if (enable) {
				if (ili9881x_ice_reg_write(dev, FLASH2_ADDR, 0x7E, 1) < 0)
					TOUCH_E("Write 0x7E at %x failed\n", FLASH2_ADDR);
			} else {
				if (ili9881x_ice_reg_write(dev, FLASH2_ADDR, 0x0, 1) < 0)
	...
	case 0xC8:
		if (flashtab->dev_id == 0x6012 || flashtab->dev_id == 0x6013) {
			if (enable) {
				if (ili9881x_ice_reg_write(dev, FLASH2_ADDR, 0x7A, 1) < 0)
...
static int ili9881x_check_ddi_chunk(struct device *dev, int from_dma, u8 *pfw)
{
	/* ili9881x_check_ddi_chunk funcion purpose is for DH15 project(DH15 f/w have more 1 block(DDI block))*/
	...
	cmd[6] = ili9881x_calc_data_checksum(cmd, sizeof(cmd) - 1);
...
dma:
	...
	len = DDI_BK_SIZE;
	if (ili9881x_read_flash_data(dev, start, end, fw_flash_tmp, len) < 0) {
...
static u32 ili9881x_read_hw_crc(struct device *dev, u32 start, u32 end)
{
	int retry = 500;
	u32 busy = 0;
	u32 write_len = end;
	...
	if (ili9881x_ice_reg_write(dev, FLASH2_ADDR, 0x3b, 1) < 0)
	...
	if (ili9881x_ice_reg_write(dev, 0x041003, 0x01, 1) < 0)
		TOUCH_E("Write enable Dio_Rx_dual failed\n");

	if (ili9881x_ice_reg_write(dev, FLASH2_ADDR, 0xFF, 1) < 0)
		TOUCH_E("Write dummy failed\n");

	if (ili9881x_ice_reg_write(dev, 0x04100C, write_len, 3) < 0)
		TOUCH_E("Write Set Receive count failed\n");

	if (ili9881x_ice_reg_write(dev, 0x048007, 0x02, 1) < 0)
		TOUCH_E("Write clearing int flag failed\n");

	if (ili9881x_ice_reg_write(dev, 0x041016, 0x00, 1) < 0)
		TOUCH_E("Write 0x0 at 0x041016 failed\n");

	if (ili9881x_ice_reg_write(dev, 0x041016, 0x01, 1) < 0)
		TOUCH_E("Write Checksum_En failed\n");

	if (ili9881x_ice_reg_write(dev, FLASH4_ADDR, 0xFF, 1) < 0)
		TOUCH_E("Write start to receive failed\n");

	do {
		if (ili9881x_ice_reg_read(dev, 0x048007, &busy, sizeof(u8)) < 0)
	...
	if (ili9881x_ice_reg_write(dev, 0x041003, 0x0, 1) < 0)
		TOUCH_E("Write disable dio_Rx_dual failed\n");

	if (ili9881x_ice_reg_read(dev, 0x04101C, &flash_crc, sizeof(u32)) < 0) {
...
static int ili9881x_check_fw_ver(struct device *dev, u8 *pfw)
{
...
//	u32 flash_crc_cb = 0,
	...
	// 	if (ili9881x_read_flash_data(dev, end_addr - crc_byte_len + 1, end_addr,
	...
	// 	hw_crc[i] = ili9881x_read_hw_crc(dev, start_addr, end_addr - start_addr - crc_byte_len + 1);
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		hw_crc[i] = ili9881x_read_hw_crc(dev, fbi[i].start, len);
...
static int ili9881x_flash_hex_hw_crc_cmp(struct device *dev, u8 *pfw)
{
	u32 i = 0, len = 0;
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		hex_crc = CalculateCRC32(fbi[i].start, len, pfw);
		hw_crc = ili9881x_read_hw_crc(dev, fbi[i].start, len);
...
static int ili9881x_flash_erase(struct device *dev)
{
	int ret = 0;
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		for (addr = fbi[i].start; addr <= fbi[i].end; addr += flashtab->sector) {
			...
			if (addr == fbi[AP].start) {
				...
				if (ili9881x_ice_reg_write(dev, FLASH2_ADDR, 0xD8, 1) < 0)
					TOUCH_E("Write 0xB at %x failed\n", FLASH2_ADDR);
				bk_erase = true;
			} else {
				/* sector erase with 4k bytes. */
				if (ili9881x_ice_reg_write(dev, FLASH2_ADDR, 0x20, 1) < 0)
					TOUCH_E("Write 0x20 at %x failed\n", FLASH2_ADDR);
			}

			recv_addr = ((addr & 0xFF0000) >> 16) | (addr & 0x00FF00) | ((addr & 0x0000FF) << 16);
			if (ili9881x_ice_reg_write(dev, FLASH2_ADDR, recv_addr, 3) < 0)
				TOUCH_E("Write address failed\n");
...
static int ili9881x_flash_program(struct device *dev, u8 *pfw)
{
	u8 buf[512] = {0};
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		for (addr = fbi[i].start; addr < fbi[i].end; addr += flashtab->program_page) {
			...
			if (ili9881x_ice_reg_write(dev, FLASH2_ADDR, 0x2, 1) < 0)
			...
			if (ili9881x_reg_write(dev, CMD_GET_MCU_STOP_INTERNAL_DATA, buf, flashtab->program_page + 3) < 0) {
...
static int ili9881x_flash_upgrade(struct device *dev, u8 *pfw)
{
	int ret = UPDATE_PASS;
	...
	ili9881x_check_ddi_chunk(dev, ret, pfw);

	ili9881x_ice_mode_enable(dev, MCU_STOP);

	ret = ili9881x_check_fw_ver(dev, pfw);
	if (ret == UPDATE_PASS)
		goto out;

	ret = ili9881x_flash_erase(dev);
	if (ret == UPDATE_FAIL)
		goto out;

	ret = ili9881x_flash_program(dev, pfw);
	if (ret == UPDATE_FAIL)
		goto out;

	ret = ili9881x_flash_hex_hw_crc_cmp(dev, pfw);
...
int ili9881x_fw_upgrade(struct device *dev, char* fwpath)
{
	int ret = 0, retry = 3;
	...
	do {
		ret = ili9881x_flash_upgrade(dev, pfw);
		if (ret == UPDATE_PASS)
			break;
...
int ili9881x_app_fw_upgrade(struct device *dev)
{
	int ret = 0, retry = 3;
...
int ili9881x_read_flash_info(struct device *dev)
{
	int i, ret = 0;
	u8 buf[4] = {0};
	u8 cmd = 0x9F;
	...
	ili9881x_ice_reg_write(dev, FLASH2_ADDR, cmd, 1);
	for (i = 0; i < ARRAY_SIZE(buf); i++) {
		ili9881x_ice_reg_write(dev, FLASH2_ADDR, 0xFF, 1);
		ili9881x_ice_reg_read(dev, FLASH4_ADDR, &tmp, sizeof(u8));

		buf[i] = tmp;
	}

	ili9881x_ice_reg_write(dev, FLASH_BASED_ADDR, 0x1, 1);
	...
	ili9881x_flash_protect(dev, false);
