...
#include "touch_ili9882n.h"
#include "touch_ili9882n_mp.h"
#include "touch_ili9882n_fw.h"
...
{
	struct ili9882n_data *d = to_ili9882n_data(dev);
...
int ili9882n_reg_read(struct device *dev, u8 cmd, void *data, int size)
{
...
int ili9882n_ice_reg_read(struct device *dev, u32 addr, void* data, int size)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	struct touch_bus_msg msg = {0, };
...
int ili9882n_reg_write(struct device *dev, u8 cmd, void *data, int size)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	struct touch_bus_msg msg = {0, };
...
int ili9882n_ice_reg_write(struct device *dev, u32 addr, u32 data, int size)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	struct touch_bus_msg msg = {0, };
...
int ili9882n_ice_mode_disable(struct device *dev)
{
	int ret = 0, retry = 0;
	...
	do {
		ret = ili9882n_reg_write(dev, CMD_ICE_MODE_EXIT, &buf, 3);
		if (ret >= 0)
			break;
...
int ili9882n_ice_mode_enable(struct device *dev, int mcu_status)
{
	int ret = 0, retry = 0;
	...
	do {
		ret = ili9882n_reg_write(dev, cmd, &buf, 3);
		if (ret)
			TOUCH_E("Failed to enter ice mode, %d\n", ret);

		ret = ili9882n_ice_reg_read(dev, ILI9882N_PID_ADDR, &pid, sizeof(pid));
		if (ret)
...
void ili9882n_ice_reg_write_bit_mask(struct device *dev, u32 addr, u32 mask, u32 value)
{
	u32 data = 0;

	TOUCH_I("%s - mask: %x, value: %x\n", __func__, mask, value);

	ili9882n_ice_reg_read(dev, addr, &data, 4);

	data &= (~mask);
	data |= (value & mask);

	ili9882n_ice_reg_write(dev, addr, data, 4);
}

void ili9882n_dma_clear(struct device *dev)
{
	TOUCH_I("%s\n", __func__);
	ili9882n_ice_reg_write_bit_mask(dev, FLASH0_ADDR, FLASH0_REG_PRECLK_SEL, (2 << 16));
	ili9882n_ice_reg_write_bit_mask(dev, FLASH4_ADDR, FLASH4_REG_FLASH_DMA_TRIGGER_EN, (0 << 24));
	ili9882n_ice_reg_write_bit_mask(dev, FLASH0_ADDR, FLASH0_REG_RX_DUAL, (0 << 24));
	ili9882n_ice_reg_write(dev, FLASH3_REG_RCV_CNT, 0x00, 1);
	ili9882n_ice_reg_write(dev, FLASH4_REG_RCV_DATA, 0xFF, 1);
}

int ili9882n_dma_read_int_flag(struct device *dev)
{
	int retry = 500;
	u32 data = 0;

	do {
		if (ili9882n_ice_reg_read(dev, INTR1_ADDR & BIT(25), &data, sizeof(u32)) < 0)
...
void ili9882n_dma_write(struct device *dev, u32 start, u32 end, u32 len)
{
	ili9882n_ice_reg_write_bit_mask(dev, FLASH0_ADDR, FLASH0_REG_PRECLK_SEL, 1 << 16);
	ili9882n_ice_reg_write(dev, FLASH0_REG_FLASH_CSB, 0x00, 1);
	ili9882n_ice_reg_write(dev, FLASH1_REG_FLASH_KEY1, 0x66aa55, 3);
	ili9882n_ice_reg_write(dev, FLASH2_REG_TX_DATA, 0x0b, 1);

	if (ili9882n_dma_read_int_flag(dev) < 0) {
		TOUCH_E("Write 0xb timeout \n");
		return;
	}

	ili9882n_ice_reg_write_bit_mask(dev, INTR1_ADDR, INTR1_REG_FLASH_INT_FLAG, (1 << 25));
	ili9882n_ice_reg_write(dev, FLASH2_REG_TX_DATA, (start & 0xFF0000) >> 16, 1);
	...
	ili9882n_ice_reg_write_bit_mask(dev, INTR1_ADDR, INTR1_REG_FLASH_INT_FLAG, (1 << 25));
	ili9882n_ice_reg_write(dev, FLASH2_REG_TX_DATA, (start & 0x00FF00) >> 8, 1);
	...
	ili9882n_ice_reg_write_bit_mask(dev, INTR1_ADDR, INTR1_REG_FLASH_INT_FLAG, (1 << 25));
	ili9882n_ice_reg_write(dev, FLASH2_REG_TX_DATA, (start & 0x0000FF), 1);
	...
	ili9882n_ice_reg_write_bit_mask(dev, INTR1_ADDR, INTR1_REG_FLASH_INT_FLAG, (1 << 25));
	ili9882n_ice_reg_write_bit_mask(dev, FLASH0_ADDR, FLASH0_REG_RX_DUAL, 0 << 24);
	ili9882n_ice_reg_write(dev, FLASH2_REG_TX_DATA, 0x00, 1);
	...
	ili9882n_ice_reg_write_bit_mask(dev, INTR1_ADDR, INTR1_REG_FLASH_INT_FLAG, (1 << 25));
	ili9882n_ice_reg_write(dev, FLASH3_REG_RCV_CNT, len, 4);
}

void ili9882n_dma_clear_reg_setting(struct device *dev)
{
	/* 1. interrupt t0/t1 enable flag */
	ili9882n_ice_reg_write_bit_mask(dev, INTR32_ADDR, INTR32_REG_T0_INT_EN | INTR32_REG_T1_INT_EN, (0 << 24));

	/* 2. clear tdi_err_int_flag */
	ili9882n_ice_reg_write_bit_mask(dev, INTR2_ADDR, INTR2_TDI_ERR_INT_FLAG_CLEAR, (1 << 18));

	/* 3. clear dma channel 0 src1 info */
	ili9882n_ice_reg_write(dev, DMA49_REG_DMA_CH0_SRC1_ADDR, 0x00000000, 4);
	ili9882n_ice_reg_write(dev, DMA50_REG_DMA_CH0_SRC1_STEP_INC, 0x00, 1);
	ili9882n_ice_reg_write_bit_mask(dev, DMA50_ADDR, DMA50_REG_DMA_CH0_SRC1_FORMAT | DMA50_REG_DMA_CH0_SRC1_EN, BIT(31));

	/* 4. clear dma channel 0 trigger select */
	ili9882n_ice_reg_write_bit_mask(dev, DMA48_ADDR, DMA48_REG_DMA_CH0_TRIGGER_SEL, (0 << 16));
	ili9882n_ice_reg_write_bit_mask(dev, INTR1_ADDR, INTR1_REG_FLASH_INT_FLAG, (1 << 25));

	/* 5. clear dma flash setting */
	ili9882n_dma_clear(dev);
}

void ili9882n_dma_trigger_reg_setting(struct device *dev, u32 reg_dest_addr, u32 flash_start_addr, u32 copy_size)
{
	int retry = 30;
	u32 stat = 0;

	/* 1. set dma channel 0 clear */
	ili9882n_ice_reg_write_bit_mask(dev, DMA48_ADDR, DMA48_REG_DMA_CH0_START_CLEAR, BIT(25));

	/* 2. set dma channel 0 src1 info */
	ili9882n_ice_reg_write(dev, DMA49_REG_DMA_CH0_SRC1_ADDR, 0x00041010, 4);
	ili9882n_ice_reg_write(dev, DMA50_REG_DMA_CH0_SRC1_STEP_INC, 0x00, 1);
	ili9882n_ice_reg_write_bit_mask(dev, DMA50_ADDR, DMA50_REG_DMA_CH0_SRC1_FORMAT | DMA50_REG_DMA_CH0_SRC1_EN, BIT(31));

	/* 3. set dma channel 0 src2 info */
	ili9882n_ice_reg_write_bit_mask(dev, DMA52_ADDR, DMA52_REG_DMA_CH0_SRC2_EN, (0 << 31));

	/* 4. set dma channel 0 dest info */
	ili9882n_ice_reg_write(dev, DMA53_REG_DMA_CH0_DEST_ADDR, reg_dest_addr, 3);
	ili9882n_ice_reg_write(dev, DMA54_REG_DMA_CH0_DEST_STEP_INC, 0x01, 1);
	ili9882n_ice_reg_write(dev, DMA54_ADDR, DMA54_REG_DMA_CH0_DEST_FORMAT | DMA54_REG_DMA_CH0_DEST_EN, BIT(31));

	/* 5. set dma channel 0 trafer info */
	ili9882n_ice_reg_write(dev, DMA55_REG_DMA_CH0_TRAFER_COUNTS, copy_size, 4);
	ili9882n_ice_reg_write_bit_mask(dev, DMA55_ADDR, DMA55_REG_DMA_CH0_TRAFER_MODE, (0 << 24));

	/* 6. set dma channel 0 int info */
	ili9882n_ice_reg_write_bit_mask(dev, INTR33_ADDR, INTR33_REG_DMA_CH0_INT_EN, (1 << 17));

	/* 7. set dma channel 0 trigger select */
	ili9882n_ice_reg_write_bit_mask(dev, DMA48_ADDR, DMA48_REG_DMA_CH0_TRIGGER_SEL, (1 << 16));

	/* 8. set dma flash setting */
	ili9882n_dma_write(dev, flash_start_addr, (flash_start_addr+copy_size), copy_size);

	/* 9. clear flash and dma ch0 int flag */
	ili9882n_ice_reg_write_bit_mask(dev, INTR1_ADDR, INTR1_REG_DMA_CH0_INT_FLAG | INTR1_REG_FLASH_INT_FLAG, BIT(17) | BIT(25));
	ili9882n_ice_reg_write_bit_mask(dev, 0x041013, BIT(0), 1);
	...
	while (retry > 0) {
		if (ili9882n_ice_reg_read(dev, INTR1_ADDR, &stat, sizeof(u32)) < 0) {
	...
	/* CS High */
	ili9882n_ice_reg_write(dev, FLASH0_REG_FLASH_CSB, 0x1, 1);
...
void ili9882n_dump_packet(void *data, int type, int len, int row_len, const char *name)
{
	int i, row = 32;
	u8 *p8 = NULL;
	int32_t *p32 = NULL;
...
int ili9882n_sense_ctrl(struct device *dev, bool start)
{
	int ret = 0;
	u8 data[2] = {0};
	...
	ret = ili9882n_reg_write(dev, CMD_CONTROL_OPTION, &data[0], sizeof(data));
	if (ret < 0)
...
int ili9882n_sleep_ctrl(struct device *dev, int command)
{
	int ret = 0;
	u8 data[2] = {0};
...
int ili9882n_plug_ctrl(struct device *dev, bool plugin)
{
	int ret = 0;
	u8 data[2] = {0};
...
int ili9882n_gesture_ctrl(struct device *dev, int tci_mode, bool swipe_enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	u8 data[3] = {0, }, dir = 0;
...
int ili9882n_ime_ctrl(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	u8 data[2] = {0};
...
int ili9882n_gesture_failreason_ctrl(struct device *dev, u8 tci_type, u8 swipe_type)
{
	int ret = 0;
	u8 data[6] = {0, };
...
void ili9882n_get_trans_touch_status(struct device *dev)
{
	u8 data[6] = {0};
	...
	int ret = 0;
	struct ili9882n_data *d = to_ili9882n_data(dev);
	struct ili9882n_core_info *core_info = &d->ic_info.core_info;
	...
	ili9882n_dump_packet(data, 8, sizeof(data), 0, "Transfer coodination");
...
int ili9882n_gesture_mode(struct device *dev, bool enable)
{
	int ret = 0;
	u8 data[2] = {0};
	...
	ret = ili9882n_reg_write(dev, CMD_READ_DATA_CTRL, &data[0], 1);
	ret = ili9882n_reg_write(dev, CMD_CONTROL_OPTION, &data[0], sizeof(data));
	if (ret < 0)
...
int ili9882n_grip_suppression_ctrl(struct device *dev, u16 x, u16 y)
{
	int ret = 0;
	u8 data[5] = {0};
...
int ili9882n_check_int_status(struct device *dev, bool high)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
int ili9882n_check_cdc_busy(struct device *dev, int count, int delay)
{
	...
	while (timer > 0) {
		...
		ili9882n_reg_write(dev, CMD_READ_DATA_CTRL, &buf, 1);
		ili9882n_reg_read(dev, CMD_CDC_BUSY_STATE, &busy, 1);
	...
	if (ret < -1) {
		...
		ili9882n_get_pc_latch(dev);
	}

	return ret;
}

void ili9882n_get_pc_latch(struct device *dev)
{
	u32 pc = 0, latch = 0;
	...
	if (ili9882n_ice_mode_enable(dev, MCU_STOP) < 0)
		TOUCH_E("Enable ice mode failed while reading pc counter\n");

	if (ili9882n_ice_reg_read(dev, ILI9882N_PC_ADDR, &pc, sizeof(u32)) < 0)
		TOUCH_E("Read pc conter error\n");

	if (ili9882n_ice_reg_read(dev, ILI9882N_LATCH_ADDR, &latch, sizeof(u32)) < 0)
		TOUCH_E("Read latch error\n");

	TOUCH_E("read pc (addr: 0x%x) = 0x%x, latch (addr: 0x%x) = 0x%x\n",
		ILI9882N_PC_ADDR, pc, ILI9882N_LATCH_ADDR, latch);

	if (ili9882n_ice_mode_disable(dev) < 0)
		TOUCH_E("Disable ice mode failed while reading pc counter\n");
}

static int ili9882n_chip_init(struct device *dev)
{
	struct ili9882n_data *d = to_ili9882n_data(dev);
	struct ili9882n_chip_info *chip_info = &d->ic_info.chip_info;
	int ret = 0;
	u32 PIDData = 0;

	TOUCH_TRACE();

	ret = ili9882n_ice_mode_enable(dev, MCU_ON);
	if (ret) {
	...
	/* Get Chip ID */
	ret = ili9882n_ice_reg_read(dev, ILI9882N_PID_ADDR, &PIDData, sizeof(PIDData));
	if (ret) {
...
out:
	ili9882n_ice_mode_disable(dev);
	return ret;
}
#if 0
static int ili9882n_fb_notifier_callback(struct notifier_block *self,
...
#endif
static int ili9882n_sw_reset(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	int ret = 0;
	...
	if(mode == SW_RESET) {
		...
		ili9882n_ice_reg_write(dev, ILI9882N_IC_RESET_ADDR, ILI9882N_IC_RESET_KEY, 4);
	} else {
...
static int ili9882n_hw_reset(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
...
int ili9882n_reset_ctrl(struct device *dev, int ctrl)
{
	TOUCH_TRACE();
	...
	switch (ctrl) {
		default :
		case SW_RESET:
			ili9882n_sw_reset(dev, ctrl);
			break;

		case HW_RESET_ASYNC:
		case HW_RESET_SYNC:
		case HW_RESET_ONLY:
			ili9882n_hw_reset(dev, ctrl);
			break;
	}
...
int ili9882n_display_power_ctrl(struct device *dev, int ctrl)
{
	TOUCH_TRACE();
...
static int ili9882n_power(struct device *dev, int ctrl)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	...
	switch (ctrl) {
		...
		case POWER_HW_RESET_ASYNC:
			...
			ili9882n_reset_ctrl(dev, HW_RESET_ASYNC);
			break;
		case POWER_HW_RESET_SYNC:
			TOUCH_I("%s, reset sync\n", __func__);
			ili9882n_reset_ctrl(dev, HW_RESET_SYNC);
			break;
		case POWER_SW_RESET:
			ili9882n_reset_ctrl(dev, SW_RESET);
			break;
		case POWER_SLEEP:
			if (d->p_param.deep_sleep_power_control == FUNC_ON) {
				ili9882n_display_power_ctrl(dev, DEEP_SLEEP_ENTER);
			}
			break;
		case POWER_WAKE:
			if (d->p_param.deep_sleep_power_control == FUNC_ON) {
				ili9882n_display_power_ctrl(dev, DEEP_SLEEP_EXIT);
			}
			break;
		case POWER_DSV_TOGGLE:
			if (d->p_param.dsv_toggle == FUNC_ON) {
				ili9882n_display_power_ctrl(dev, DSV_TOGGLE);
			}
			break;
		case POWER_DSV_ALWAYS_ON:
			if (d->p_param.dsv_toggle == FUNC_ON) {
				ili9882n_display_power_ctrl(dev, DSV_ALWAYS_ON);
			}
			break;
		default:
...
static void ili9882n_get_tci_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
int ili7807_get_fw_data_wrapper(struct device *dev, u8 *wdata, int wsize, void *rdata, int rsize) {
	...
	/* pre cmd */
	ret = ili9882n_reg_write(dev, CMD_READ_DATA_CTRL, &temp, 1);


	/* cmd */
	ret = ili9882n_reg_write(dev, wdata[0], ptr, wsize);
	if (ret < 0) {
	...
	ret = ili9882n_reg_read(dev, CMD_NONE, rdata, rsize);
	if (ret < 0) {
	...
	ili9882n_dump_packet(rdata, 8, rsize, 16, str);

	return ret;
}


int ili9882n_tp_info(struct device *dev)
{
	struct ili9882n_data *d = to_ili9882n_data(dev);
	struct ili9882n_panel_info *panel_info = &d->tp_info.panel_info;
	//struct ili9882n_panel_extra_info *panel_extra_info = &d->tp_info.panel_extra_info;
	int ret = 0;
	u8 cmd;
	...
	/* GEt TP info */
	// ret = ili9882n_reg_read(dev, CMD_GET_TP_INFORMATION, panel_info, sizeof(*panel_info));
...
int ili9882n_ic_info(struct device *dev)
{
	struct ili9882n_data *d = to_ili9882n_data(dev);
	struct ili9882n_fw_info *fw_info = &d->ic_info.fw_info;
	struct ili9882n_protocol_info *protocol_info = &d->ic_info.protocol_info;
	struct ili9882n_core_info *core_info = &d->ic_info.core_info;
	/*No need build version */
	// struct ili9882n_fw_extra_info *fw_extra_info = &d->ic_info.fw_extra_info;
	//struct ili9882n_panel_extra_info *panel_extra_info = &d->tp_info.panel_extra_info;
	int ret = 0;
	u8 cmd;
	...
	/* Get Fw info */
	// ret = ili9882n_reg_read(dev, CMD_GET_FIRMWARE_VERSION, fw_info, sizeof(*fw_info));
	...
	/*No need in this project*/
	// /* Get FW build info */
	// ret = ili9882n_reg_read(dev, CMD_GET_FRIMWARE_BUILD_VERSION, &(fw_extra_info->build), sizeof(u8));
	// if (ret) {
	...
	/* Get Protocol ver */
	// ret = ili9882n_reg_read(dev, CMD_GET_PROTOCOL_VERSION, protocol_info, sizeof(*protocol_info));
	...
	/* Get FW core ver */
	// ret = ili9882n_reg_read(dev, CMD_GET_CORE_VERSION, core_info, sizeof(core_info));
	...
	ili9882n_get_trans_touch_status(dev);

	/*No need in this project*/
	/* Get Tp extra info(signal_info, glass_id_info) */
	// ret = ili9882n_reg_read(dev, CMD_GET_TP_SIGNAL_INFORMATION, &(panel_extra_info->signal), sizeof(u8));
	// if (ret) {
	// 	TOUCH_E("Failed to read touch panel signal information, %d\n", ret);
	// 	goto error;
	// }
...
#ifdef LG_KNOCK_CODE
static int ili9882n_tci_password(struct device *dev)
{
	return ili9882n_tci_knock(dev);
}
#endif

static int ili9882n_lpwg_control(struct device *dev, int tci_mode, bool swipe_enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	int ret = 0;
	...
	if (tci_mode >= LPWG_NONE && tci_mode <= LPWG_PASSWORD_ONLY) {
		TOUCH_I("ili9882n_lpwg_control - tci_mode = %s, swipe_enable = %s\n",
	...
	if (tci_mode || swipe_enable) {
		ret = ili9882n_gesture_ctrl(dev, tci_mode, swipe_enable);
		ret = ili9882n_gesture_failreason_ctrl(dev, d->tci_debug_type, d->swipe_debug_type);
		ret = ili9882n_gesture_mode(dev, true);
	} else {
		ret = ili9882n_gesture_mode(dev, false);
	}

	return ret;
}

static int ili9882n_deep_sleep_ctrl(struct device *dev, int state)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	int ret = 0;
	...
	if (!atomic_read(&ts->state.incoming_call)) { /* IDLE status */
		if (state == IC_DEEP_SLEEP) { // IC_DEEP_SLEEP
			if (d->actual_fw_mode != FIRMWARE_GESTURE_MODE) {
				if (d->sense_stop) {
					ili9882n_sense_ctrl(dev, false);
					ili9882n_check_cdc_busy(dev, 10, 50);
				}
			}
			ili9882n_sleep_ctrl(dev, SLEEP_DEEP);
			touch_interrupt_control(ts->dev, INTERRUPT_DISABLE);
			touch_msleep(150);

			atomic_set(&ts->state.sleep, IC_DEEP_SLEEP);

			ili9882n_power(dev, POWER_SLEEP); // Reset Low
			d->actual_fw_mode = FIRMWARE_UNKNOWN_MODE;
		} else { // IC_NORMAL
			ili9882n_power(dev, POWER_WAKE); // Reset High
			...
			//touch_msleep(ts->caps.hw_reset_delay);
			ili9882n_reset_ctrl(dev, HW_RESET_ONLY);
			ili9882n_ice_mode_enable(dev, MCU_ON);
			ili9882n_ice_reg_write(dev, ILI9882N_IC_RESET_GESTURE_ADDR,
					ILI9882N_IC_RESET_GESTURE_KEY, sizeof(u32));
			ili9882n_reset_ctrl(dev, HW_RESET_ONLY);
			...
			ili9882n_ice_mode_enable(dev, MCU_ON);
			while (timeout > 0) {
				ili9882n_ice_reg_read(dev, ILI9882N_IC_RESET_GESTURE_ADDR, &value, sizeof(value));

				if (value == ILI9882N_IC_RESET_GESTURE_RUN) {
			...
			ili9882n_gesture_ctrl(dev, ts->lpwg.mode,
				ts->swipe[SWIPE_U].enable || ts->swipe[SWIPE_L].enable || ts->swipe[SWIPE_R].enable);
			ili9882n_gesture_failreason_ctrl(dev, d->tci_debug_type, d->swipe_debug_type);
			ili9882n_switch_fw_mode(dev, FIRMWARE_GESTURE_MODE);
		}
		...
		if (state == IC_DEEP_SLEEP) { // IC_DEEP_SLEEP
			...
			ili9882n_sleep_ctrl(dev, SLEEP_IN);
			...
			touch_interrupt_control(ts->dev, INTERRUPT_ENABLE);
			atomic_set(&ts->state.sleep, IC_NORMAL);
			ili9882n_lpwg_control(dev, ts->lpwg.mode,
				(ts->swipe[SWIPE_U].enable || ts->swipe[SWIPE_L].enable || ts->swipe[SWIPE_R].enable));
...
int ili9882n_report_tci_fr_buffer(struct device *dev)
{
	struct ili9882n_data *d = to_ili9882n_data(dev);
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0, i;
	...
	ret = ili9882n_reg_read(dev, CMD_NONE, tci_fr_buffer, sizeof(tci_fr_buffer));
	if (ret < 0) {
		TOUCH_E("Failed to read buffer type fail reason\n");
	}

	ili9882n_dump_packet(tci_fr_buffer, 8, sizeof(tci_fr_buffer), 16, "buffer type");
...
int ili9882n_report_swipe_fr_buffer(struct device *dev)
{
	struct ili9882n_data *d = to_ili9882n_data(dev);
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0, i;
	...
	ret = ili9882n_reg_read(dev, CMD_NONE, swipe_fr_buffer, sizeof(swipe_fr_buffer));
	if (ret < 0) {
		TOUCH_E("Failed to read buffer type fail reason\n");
	}

	ili9882n_dump_packet(swipe_fr_buffer, 8, sizeof(swipe_fr_buffer), 16, "buffer type");
...
static int ili9882n_lpwg_mode(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	int ret = 0;
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		if (ts->mfts_lpwg) {
			...
			ili9882n_lpwg_control(dev, LPWG_DOUBLE_TAP, false);
			ili9882n_switch_fw_mode(dev, FIRMWARE_GESTURE_MODE);
			return 0;
		}
		...
		if (ts->lpwg.screen) {
			...
			if(d->tci_debug_type & FAIL_BUFFER_TYPE)
				ili9882n_report_tci_fr_buffer(dev); // Report fr before touch IC reset
			if(d->swipe_debug_type & FAIL_BUFFER_TYPE)
				ili9882n_report_swipe_fr_buffer(dev); // Report fr before touch IC reset
			ili9882n_power(dev, POWER_DSV_ALWAYS_ON);
		} else if (ts->lpwg.sensor == PROX_NEAR) {
			/* deep sleep */
			TOUCH_I("suspend sensor == PROX_NEAR\n");
			if (atomic_read(&ts->state.sleep) != IC_DEEP_SLEEP)
				ret = ili9882n_deep_sleep_ctrl(dev, IC_DEEP_SLEEP);
			...
			/* knock on/code */
			if (atomic_read(&ts->state.sleep) == IC_DEEP_SLEEP) {
				ret = ili9882n_deep_sleep_ctrl(dev, IC_NORMAL);
			} else {
				...
				ili9882n_switch_fw_mode(dev, FIRMWARE_GESTURE_MODE);
				if(d->p_param.dsv_toggle == FUNC_ON) {
					ili9882n_power(dev, POWER_DSV_TOGGLE);
				}
			}
...
static int ili9882n_lpwg(struct device *dev, u32 code, void *param)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int *value = (int *)param;
	int ret = 0;
...
static void ili9882n_connect(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	...
	ili9882n_plug_ctrl(dev, d->charger);
}
#if 0
static void ili9882n_lcd_mode(struct device *dev, u32 mode)
{
	struct ili9882n_data *d = to_ili9882n_data(dev);
...
#endif
static int ili9882n_usb_status(struct device *dev, u32 mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	ili9882n_connect(dev);
	return 0;
}

static int ili9882n_wireless_status(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int ili9882n_earjack_status(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
#if 0
static void ili9882n_fb_notify_work_func(struct work_struct *fb_notify_work)
{
	struct ili9882n_data *d = container_of(to_delayed_work(fb_notify_work),
			struct ili9882n_data, fb_notify_work);
	int ret = 0;
...
static int ili9882n_notify_charger(struct device *dev, ulong event, void *data)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	int ret = 0;
...
#endif
static int ili9882n_notify_normal(struct device *dev, ulong event, void *data)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	...
	switch (event) {
...
#else
			TOUCH_I("CONFIG_LGE_TOUCH_USE_PANEL_NOTIFY is not set\n");
#endif
			break;
		...
		case NOTIFY_CONNECTION:
			...
			ret = ili9882n_usb_status(dev, *(u32 *)data);
			break;

		case NOTIFY_WIRELESS:
			TOUCH_I("NOTIFY_WIRELESS!\n");
			ret = ili9882n_wireless_status(dev, *(u32 *)data);
			break;

		case NOTIFY_EARJACK:
			TOUCH_I("NOTIFY_EARJACK!\n");
			ret = ili9882n_earjack_status(dev, *(u32 *)data);
			break;

		case NOTIFY_IME_STATE:
			TOUCH_I("NOTIFY_IME_STATE!\n");
			ret = ili9882n_ime_ctrl(dev);
			break;
		...
		case NOTIFY_CALL_STATE:
			...
			if (*(u32*)data >= INCOMING_CALL_IDLE && *(u32*)data <= INCOMING_CALL_LTE_OFFHOOK) {
				...
				//ret = ili9882n_reg_write(dev, SPECIAL_CALL_INFO_CTRL, (u32*)data, sizeof(u32));
			}
			break;
...
static int ili9882n_notify(struct device *dev, ulong event, void *data)
{
	TOUCH_TRACE();
#if 0
	if (touch_check_boot_mode(dev) == TOUCH_CHARGER_MODE) {
		TOUCH_I("CHARGER MODE notify\n");
		return ili9882n_notify_charger(dev,event,data);
	}
#endif
	return ili9882n_notify_normal(dev,event,data);
}

static void ili9882n_init_works(struct ili9882n_data *d)
{
//	INIT_DELAYED_WORK(&d->fb_notify_work, ili9882n_fb_notify_work_func);
	init_waitqueue_head(&(d->inq));
}

static void ili9882n_init_locks(struct ili9882n_data *d)
{
...
static int ili9882n_probe(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = NULL;
	int ret = 0;
	...
	/*******************************************************
	 * Display driver does control the power in ili9882n IC *
	...
	ili9882n_init_works(d);
	ili9882n_init_locks(d);
	...
	/* Get flash size info */
	ret = ili9882n_read_flash_info(dev);
	if (ret) {
	...
	ili9882n_get_tci_info(dev);

	/* Initialize ILITEK's APK for fw debug */
	ili9882n_apk_init(dev);
	...
	d->swipe_debug_type = FAIL_REASON_OFF;

	return ret;
}

static int ili9882n_remove(struct device *dev)
{
	TOUCH_TRACE();
...
static int ili9882n_shutdown(struct device *dev)
{
	TOUCH_TRACE();
...
static int ili9882n_upgrade(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	//struct ili9882n_data *d = to_ili9882n_data(dev);
	//struct ili9882n_panel_extra_info *panel_extra_info = &d->tp_info.panel_extra_info;
...
//-------------------------------------------------
	...
	ret = ili9882n_fw_upgrade(dev, fwpath);
	if (ret)
...
static int ili9882n_app_upgrade(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	...
	ret = ili9882n_app_fw_upgrade(dev);
	if (ret)
		TOUCH_E("fail to firmware upgrade");
...
static int ili9882n_suspend(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	int mfts_mode = 0;
...
static int ili9882n_resume(struct device *dev)
{
	...
	mfts_mode = touch_check_boot_mode(dev);
	if ((mfts_mode >= TOUCH_MINIOS_MFTS_FOLDER && mfts_mode <= TOUCH_MINIOS_MFTS_CURVED) && !ts->mfts_lpwg) {
		ili9882n_power(dev, POWER_ON);
		touch_msleep(ts->caps.hw_reset_delay);
		ili9882n_ic_info(dev);
		if (ili9882n_upgrade(dev) == 0) {
			ili9882n_power(dev, POWER_OFF);
			ili9882n_power(dev, POWER_ON);
			touch_msleep(ts->caps.hw_reset_delay);
		}
...
#endif
	if (touch_check_boot_mode(dev) == TOUCH_CHARGER_MODE) {
		ili9882n_deep_sleep_ctrl(dev, IC_DEEP_SLEEP);
		return -EPERM;
	}
...
static int ili9882n_init(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	int ret = 0;
	...
	if (atomic_read(&ts->state.core) == CORE_PROBE) {
		...
		ts->fb_notif.notifier_call = ili9882n_fb_notifier_callback;
		fb_register_client(&ts->fb_notif);
	}
#endif
	ret = ili9882n_ic_info(dev);
	if (ret) {
		TOUCH_E("ili9882n_ic_info failed, ret:%d\n", ret);
		return ret;
	}
...
int ili9882n_switch_fw_mode(struct device *dev, u8 mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	if (mode == FIRMWARE_DEMO_MODE || mode == FIRMWARE_DEBUG_MODE) {
		...
		if (ili9882n_reg_write(dev, CMD_MODE_CONTROL, &mode, sizeof(mode)) < 0) {
		...
		ili9882n_reg_read(dev, CMD_READ_MP_TEST_CODE_INFO, mp_code, codeLength);

		ili9882n_dump_packet(mp_code, 8, codeLength, 0, "MP overlay info");
		...
		if (ili9882n_check_cdc_busy(dev, 10, 50) < 0) {
		...
		ili9882n_dma_clear_reg_setting(dev);

		TOUCH_I("[Move ANDES.INIT to DRAM]\n");
		ili9882n_dma_trigger_reg_setting(dev, 0, mp_flash_addr, mp_andes_init_size);

		ili9882n_dma_clear_reg_setting(dev);

		TOUCH_I("[Move MP.TEXT to DRAM]\n");
		ili9882n_dma_trigger_reg_setting(dev, overlay_end_addr, (mp_flash_addr + overlay_start_addr), mp_text_size);

		ili9882n_dma_clear_reg_setting(dev);

		/* Code reset */
		ili9882n_ice_reg_write(dev, 0x040040, 0xAE, 1);
		...
		if (ili9882n_check_cdc_busy(dev, 300, 50) < 0) {
...
u8 ili9882n_calc_data_checksum(void *pMsg, u32 nLength)
{
	int i;
	u8 *p8 = NULL;
	u32 nCheckSum = 0;
...
static int ili9882n_get_gesture_data(struct device *dev, int count)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
...
static int ili9882n_get_swipe_data(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	u8 *data = d->gesture_info.data;
...
static int ili9882n_irq_uart(struct device *dev, u32 len, void *data)
{
	struct ili9882n_data *d = to_ili9882n_data(dev);
	u8 *buf;
	int type = 0;
	...
	ret = ili9882n_reg_read(dev, CMD_NONE, &d->debug_info.uart[len], uart_len);
	if (ret) {
...
static int ili9882n_irq_abs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	struct ili9882n_touch_abs_data *abs_data = d->touch_info.abs_data;
	//struct ili9882n_touch_shape_data *shape_data = d->touch_info.shape_data;
	...
	ts->new_mask = 0;
	memset(&d->touch_info, 0x0, sizeof(struct ili9882n_touch_info));

	ret = ili9882n_reg_read(dev, CMD_NONE, &d->touch_info, sizeof(d->touch_info));
	if (ret) {
	...
	ili9882n_dump_packet(&d->touch_info, 8, sizeof(d->touch_info), 8, "DEMO");
	...
	if (d->touch_info.packet_id == I2CUART_PACKET_ID) {
		ili9882n_irq_uart(dev, sizeof(d->touch_info), &d->touch_info);
		return ret;
	}

	//Compare checksum
	check_sum = ili9882n_calc_data_checksum(&d->touch_info, (sizeof(d->touch_info) - 1));
...
static void ili9882n_irq_lpwg_failreason(struct device *dev)
{
	struct ili9882n_data *d = to_ili9882n_data(dev);
	u8 *data = d->gesture_info.data;
...
static void ili9882n_irq_lpwg_gesture(struct device *dev, int gcode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	switch (gcode) {
		case GESTURE_DOUBLECLICK:
			if (ts->lpwg.mode != LPWG_NONE) {
				ili9882n_get_gesture_data(dev, ts->tci.info[TCI_1].tap_count);
...
int ili9882n_irq_lpwg(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	u8 *data = d->gesture_info.data;
	...
	ts->new_mask = 0;
	memset(&d->gesture_info, 0x0, sizeof(struct ili9882n_gesture_info));

	ret = ili9882n_reg_read(dev, CMD_NONE, &d->gesture_info, sizeof(d->gesture_info));
	if (ret < 0) {
		TOUCH_E("touch_info read fail\n");
		goto error;
	}

	ili9882n_dump_packet(data, 8, sizeof(d->gesture_info), 8, "Gesture");

	check_sum = ili9882n_calc_data_checksum(&d->gesture_info, (sizeof(d->gesture_info) - 1));
	...
	switch (pid) {
		case GESTURE_PACKET_ID:
			ili9882n_irq_lpwg_gesture(dev, gcode);
			break;
		case FAILREASON_PACKET_ID:
			ili9882n_irq_lpwg_failreason(dev);
			break;
		case I2CUART_PACKET_ID:
			ili9882n_irq_uart(dev, sizeof(d->gesture_info), &d->gesture_info);
			break;
		default:
...
int ili9882n_irq_debug(struct device *dev) 
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	struct touch_data *tdata;
	...
	ret = ili9882n_reg_read(dev, CMD_NONE, &d->debug_info.data,
	...
	if (d->touch_info.packet_id == I2CUART_PACKET_ID) {
		ili9882n_irq_uart(dev, DEBUG_MODE_PACKET_HEADER_LENGTH, &d->debug_info.data);
		return ret;
	}

	debug_len = d->debug_info.data[1] << 8 | d->debug_info.data[2];
	TOUCH_D(ABS, "debug_length = %d\n", debug_len);
	ret = ili9882n_reg_read(dev, CMD_NONE, &(d->debug_info.data[5]),
	...
	// for touch data debug
	ili9882n_dump_packet(data, 8, sizeof(d->debug_info.data), 8, "DEBUG");
...
int ili9882n_irq_handler(struct device *dev)
{
	struct ili9882n_data *d = to_ili9882n_data(dev);
	int ret = 0;
	...
	switch (d->actual_fw_mode) {
		case FIRMWARE_DEMO_MODE:
			ret = ili9882n_irq_abs(dev);
			break;
		case FIRMWARE_DEBUG_MODE:
			ret = ili9882n_irq_debug(dev);
			break;
		case FIRMWARE_GESTURE_MODE:
			if (d->gesture_debug)
				ret = ili9882n_irq_debug(dev);
			else
				ret = ili9882n_irq_lpwg(dev);
			break;
		default:
...
static ssize_t show_reg_ctrl(struct device *dev, char *buf)
{
	...
	if (!strcmp(d->register_ctrl.cmd, "write")) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "write reg[%x] = %d\n",
				d->register_ctrl.reg,
				d->register_ctrl.data);
		TOUCH_I("write reg[%x] = %d\n",
				d->register_ctrl.reg,
				d->register_ctrl.data);
	} else if (!strcmp(d->register_ctrl.cmd, "read")) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "read reg[%x] = %d\n",
				d->register_ctrl.reg,
				d->register_ctrl.data);
		TOUCH_I("read reg[%x] = %d\n",
				d->register_ctrl.reg,
				d->register_ctrl.data);
	} else if (!strcmp(d->register_ctrl.cmd, "fail")) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "reg read/write fail\n");
		TOUCH_E("reg read/write fail\n");
//	} else if (!strcmp(d->register_ctrl.cmd, "error")) {
	} else {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "Usage\n");
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "Write reg value\n");
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "Read reg\n");
		TOUCH_E("Usage\n");
...
static ssize_t store_reg_ctrl(struct device *dev, const char *buf, size_t count)
{
	...
	if (!strcmp(command, "write")) {
		if (ili9882n_reg_write(dev, cmd8, &data, sizeof(u32)) < 0) {
			TOUCH_E("reg addr 0x%x write fail\n", cmd8);
			snprintf(d->register_ctrl.cmd, REG_CMD_SIZE, "fail");
		} else {
			TOUCH_I("reg[%x] = 0x%x\n", cmd8, data);
			snprintf(d->register_ctrl.cmd, REG_CMD_SIZE, "write");
			d->register_ctrl.reg = cmd8;
			d->register_ctrl.data = data;
		}
	} else if (!strcmp(command, "read")) {
		if (ili9882n_reg_read(dev, cmd8, &data, sizeof(u32)) < 0) {
			TOUCH_E("reg addr 0x%x read fail\n", cmd8);
			snprintf(d->register_ctrl.cmd, REG_CMD_SIZE, "fail");
		} else {
			TOUCH_I("reg[%x] = 0x%x\n", cmd8, data);
			snprintf(d->register_ctrl.cmd, REG_CMD_SIZE, "read");
			d->register_ctrl.reg = cmd8;
			d->register_ctrl.data = data;
		}
	} else {
		...
		snprintf(d->register_ctrl.cmd, REG_CMD_SIZE, "error");
	}
	return count;
}
...
static ssize_t store_grip_suppression(struct device *dev, const char *buf, size_t count)
{
	...
	ret = ili9882n_grip_suppression_ctrl(dev, value_x, value_y);
	if (ret < 0)
...
#if 0
int ili9882n_get_glass_id(struct device *dev)
{
	struct ili9882n_data *d = to_ili9882n_data(dev);
	struct ili9882n_panel_extra_info *panel_extra_info = &d->tp_info.panel_extra_info;
	...
	ili9882n_ice_reg_write(dev, 0x041000, 0x0, 1);  /* CS low */
	ili9882n_ice_reg_write(dev, 0x041004, 0x66AA55, 3); /* Key */

	ili9882n_ice_reg_write(dev, 0x041008, 0x03, 1);

	ili9882n_ice_reg_write(dev, 0x041008, (addr & 0xFF0000) >> 16, 1);
	ili9882n_ice_reg_write(dev, 0x041008, (addr & 0x00FF00) >> 8, 1);
	ili9882n_ice_reg_write(dev, 0x041008, (addr & 0x0000FF), 1);

	for (i = 0; i < GLASS_ID_SIZE; i++) {
		ili9882n_ice_reg_write(dev, 0x041008, 0xFF, 1);
		ili9882n_ice_reg_read(dev, 0x41010, &glass_id[i], sizeof(u8));
	}
	...
	ili9882n_ice_reg_write(dev, 0x041000, 0x1, 1); /* CS high */

	ret = ili9882n_ice_mode_disable(dev);
	if(ret < 0){
...
#endif
static TOUCH_ATTR(reg_ctrl, show_reg_ctrl, store_reg_ctrl);
...
static struct attribute *ili9882n_attribute_list[] = {
...
static const struct attribute_group ili9882n_attribute_group = {
	.attrs = ili9882n_attribute_list,
};

static int ili9882n_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	TOUCH_TRACE();

	ret = sysfs_create_group(&ts->kobj, &ili9882n_attribute_group);
	if (ret) {
		TOUCH_E("ili9882n sysfs register failed\n");

		goto error;
	}

	ili9882n_mp_register_sysfs(dev);
	if (ret) {
		TOUCH_E("ili9882n register failed\n");
...
static int ili9882n_get_cmd_version(struct device *dev, char *buf)
{
	struct ili9882n_data *d = to_ili9882n_data(dev);
	struct ili9882n_fw_info *fw_info = &d->ic_info.fw_info;
	struct ili9882n_protocol_info *protocol_info = &d->ic_info.protocol_info;
...
static int ili9882n_get_cmd_atcmd_version(struct device *dev, char *buf)
{
	struct ili9882n_data *d = to_ili9882n_data(dev);
	struct ili9882n_fw_info *fw_info = &d->ic_info.fw_info;
	int offset = 0;
...
static int ili9882n_swipe_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);

	TOUCH_TRACE();

	ili9882n_lpwg_control(dev, ts->lpwg.mode, enable);

	return 0;
}

static int ili9882n_init_pm(struct device *dev)
{
#if 0
	...
	ts->drm_notif.notifier_call = ili9882n_drm_notifier_callback;
...
static int ili9882n_esd_recovery(struct device *dev)
{
	TOUCH_TRACE();
#if 0
#if defined(CONFIG_LGE_TOUCH_CORE_QCT)
...
static int ili9882n_set(struct device *dev, u32 cmd, void *input, void *output)
{
	TOUCH_TRACE();
...
static int ili9882n_get(struct device *dev, u32 cmd, void *input, void *output)
{
	int ret = 0;
	...
	switch (cmd) {
		case CMD_VERSION:
			ret = ili9882n_get_cmd_version(dev, (char *)output);
			break;

		case CMD_ATCMD_VERSION:
			ret = ili9882n_get_cmd_atcmd_version(dev, (char *)output);
			break;
...
static struct touch_driver touch_driver = {
	.probe = ili9882n_probe,
	.remove = ili9882n_remove,
	.shutdown = ili9882n_shutdown,
	.suspend = ili9882n_suspend,
	.resume = ili9882n_resume,
	.init = ili9882n_init,
	.irq_handler = ili9882n_irq_handler,
	.power = ili9882n_power,
	.upgrade = ili9882n_upgrade,
#if defined(CONFIG_LGE_TOUCH_APP_FW_UPGRADE)
	.app_upgrade = ili9882n_app_upgrade,
#endif
	.esd_recovery = ili9882n_esd_recovery,
	.lpwg = ili9882n_lpwg,
	.swipe_enable = ili9882n_swipe_enable,
	.notify = ili9882n_notify,
	.init_pm = ili9882n_init_pm,
	.register_sysfs = ili9882n_register_sysfs,
	.set = ili9882n_set,
	.get = ili9882n_get,
};

#define MATCH_NAME	"lge,ili9882n"
...
static int __init touch_device_init(void)
{
	...
	TOUCH_I("%s, lcm_name = %s\n", __func__, lge_get_lcm_name());

	return touch_bus_device_init(&hwif, &touch_driver);

#if 0 // for FH10 demigod bring up
	if (is_lcm_name("SKYWORTH-ILI9882N")) {
		TOUCH_I("%s, ILI9882N found!!lcm_name = %s\n",__func__,lge_get_lcm_name());
		return touch_bus_device_init(&hwif, &touch_driver);
	}

	TOUCH_I("%s, ili9882n not found\n", __func__);
