...
#ifndef LGE_TOUCH_ILI9882N_H
#define LGE_TOUCH_ILI9882N_H

#define CHIP_ID				0x9882
...
#define ILI9882N_SLAVE_I2C_ADDR		0x41
#define ILI9882N_ICE_MODE_ADDR		0x181062
#define ILI9882N_PID_ADDR		0x4009C
#define ILI9882N_WDT_ADDR		0x5100C
#define ILI9882N_PC_ADDR		0x44008
#define ILI9882N_LATCH_ADDR		0x51010
#define ILI9882N_IC_RESET_ADDR		0x040050
#define ILI9882N_IC_RESET_KEY		0x00019881
#define ILI9882N_IC_RESET_GESTURE_ADDR	0x04005C //0x040054
#define ILI9882N_IC_RESET_GESTURE_KEY	0xF38A //0xF38A94EF
#define ILI9882N_IC_RESET_GESTURE_RUN	0xA67C //0xA67C9DFE
...
struct ili9882n_touch_abs_data {
	u8 y_high:4;
	u8 x_high:4;
	u8 x_low;
	u8 y_low;
...
/*
struct ili9882n_touch_shape_data {
	s8 degree;
	u8 width_major_high;
	u8 width_major_low;
	u8 width_minor_high;
...
*/
/* report packet */
struct ili9882n_touch_info {
	u8 packet_id;
	struct ili9882n_touch_abs_data abs_data[10];
	u8 p_sensor:4;
	u8 key:4;
	//struct ili9882n_touch_shape_data shape_data[10];
	u8 checksum;
} __packed;

struct ili9882n_debug_info {
...
struct ili9882n_gesture_info {
	u8 data[GESTURE_MODE_PACKET_INFO_LENGTH - 1];
	u8 checksum;
} __packed;

struct ili9882n_chip_info {
	u32 pid;
	u16 id;
	u8 type;
	u8 ver;
...
struct ili9882n_fw_extra_info {
	u8 build;
} __packed;
struct ili9882n_protocol_info {
	u8 command_id;
	u8 major;
	u8 mid;
	u8 minor;
...
struct ili9882n_ic_info {
	struct ili9882n_chip_info chip_info;
	struct ili9882n_fw_info fw_info;
	struct ili9882n_fw_extra_info fw_extra_info;
	struct ili9882n_protocol_info protocol_info;
	struct ili9882n_core_info core_info;
};

struct ili9882n_panel_info {
	u8 command_id;
	u8 nMinX;
	u8 nMinY;
	u8 nMaxX_Low;
...
#if 0
struct ili9882n_panel_extra_info {
...
struct ili9882n_tp_info {
	struct ili9882n_panel_info panel_info;
//	struct ili9882n_panel_extra_info panel_extra_info;
};

#define REG_CMD_SIZE 6
struct sw42902_register_ctrl {
	char cmd[REG_CMD_SIZE];
	u32 page;
	u32 reg;
	u32 offset;
	u32 data;
...
//	struct delayed_work fb_notify_work;
	...
	struct ili9882n_ic_info ic_info;
	struct ili9882n_tp_info tp_info;
	struct ili9882n_touch_info touch_info;
	struct ili9882n_debug_info debug_info;
	struct ili9882n_gesture_info gesture_info;
	struct sw42902_register_ctrl register_ctrl;
...
extern void ili9882n_get_pc_latch(struct device *dev);
extern int ili9882n_dump_flash_data(struct device *dev, u32 start, u32 end);
extern void ili9882n_dump_iram_data(struct device *dev, u32 start, u32 end);
extern int ili9882n_switch_fw_mode(struct device *dev, u8 mode);
extern int ili9882n_ice_mode_disable(struct device *dev);
extern int ili9882n_ice_mode_enable(struct device *dev, int mcu_status);
extern int ili9882n_ice_reg_write(struct device *dev, u32 addr, u32 data, int size);
extern int ili9882n_ice_reg_read(struct device *dev, u32 addr, void* data, int size);
extern int ili9882n_reg_read(struct device *dev, u8 cmd, void *data, int size);
extern int ili9882n_reg_write(struct device *dev, u8 cmd, void *data, int size);
extern void ili9882n_get_trans_report(struct device *dev);
extern int ili9882n_tp_info(struct device *dev);
extern int ili9882n_reset_ctrl(struct device *dev, int ctrl);
extern int ili9882n_ic_info(struct device *dev);
extern int ili9882n_check_cdc_busy(struct device *dev, int conut, int delay);
extern void ili9882n_dump_packet(void *data, int type, int len, int row_len, const char *name);
extern u8 ili9882n_calc_data_checksum(void *pMsg, u32 nLength);
// int ili9882n_ic_info(struct device *dev);
// int ili9882n_tc_driving(struct device *dev, int mode);
// int ili9882n_irq_abs(struct device *dev);
// int ili9882n_irq_lpwg(struct device *dev);
// int ili9882n_irq_handler(struct device *dev);
// int ili9882n_check_status(struct device *dev);
// int ili9882n_debug_info(struct device *dev);

static inline struct ili9882n_data *to_ili9882n_data(struct device *dev)
{
	return (struct ili9882n_data *)touch_get_device(to_touch_core(dev));
}

static inline struct ili9882n_data *to_ili9882n_data_from_kobj(struct kobject *kobj)
{
	return (struct ili9882n_data *)container_of(kobj,
			struct ili9882n_data, kobj);
}
static inline int ili9882n_read_value(struct device *dev,
		u16 addr, u32 *value)
{
	return ili9882n_reg_read(dev, addr, value, sizeof(*value));
}

static inline int ili9882n_write_value(struct device *dev,
		u16 addr, u32 value)
{
	return ili9882n_reg_write(dev, addr, &value, sizeof(value));
}
...
extern int ili9882n_apk_init(struct device *dev);
extern int ili9882n_check_int_status(struct device *dev, bool high);
extern int ili7807_get_fw_data_wrapper(struct device *dev, u8 *wdata, int wsize, void *rdata, int rsize);
extern void ili9882n_get_ddi_reg_onepage(u8 page, u8 reg, u8 *data);

#endif /* LGE_TOUCH_ILI9882N_H */
