...
static struct ili9882n_data *touch_dev = NULL;
...
static void ili9882n_ddi_wr_pack(int packet)
{
	int retry = 100;
	u32 reg_data = 0;

	while (retry--) {
		if (ili9882n_ice_reg_read(touch_dev->dev, 0x73010, &reg_data, sizeof(u8)) < 0)
	...
	if (ili9882n_ice_reg_write(touch_dev->dev, 0x73000, packet, 4) < 0)
		TOUCH_E("Write %x at 0x73000\n", packet);
}

static u8 ili9882n_ddi_rd_pack(int packet)
{
	int retry = 100;
	u8 reg_data = 0;

	ili9882n_ddi_wr_pack(packet);
	while (retry--) {
		if (ili9882n_ice_reg_read(touch_dev->dev, 0x4800A, &reg_data, sizeof(u8)) < 0)
	...
	if (ili9882n_ice_reg_write(touch_dev->dev, 0x4800A, 0x02, 1) < 0)
		TOUCH_E("Write 0x2 at 0x4800A\n");

	if (ili9882n_ice_reg_read(touch_dev->dev, 0x73016, &reg_data, sizeof(u8)) < 0)
		TOUCH_E("Read 0x73016 error\n");

	return reg_data;
}

static void ili9882n_set_ddi_reg_onepage(u8 page, u8 reg, u8 data)
{
	...
	if (ili9882n_ice_mode_enable(touch_dev->dev, MCU_STOP) < 0)
		TOUCH_E("Enable ice mode failed before writing ddi reg\n");

	/*TDI_WR_KEY*/
	ili9882n_ddi_wr_pack(0x1FFF9527);
	/*Switch to Page*/
	ili9882n_ddi_wr_pack(setpage);
	/* Page*/
	ili9882n_ddi_wr_pack(setreg);
	/*TDI_WR_KEY OFF*/
	ili9882n_ddi_wr_pack(0x1FFF9500);

	if (ili9882n_ice_mode_disable(touch_dev->dev) < 0)
		TOUCH_E("Disable ice mode failed after writing ddi reg\n");
}

void ili9882n_get_ddi_reg_onepage(u8 page, u8 reg, u8 *data)
{
	u32 setpage = 0x1FFFFF00 | page;
	...
	ili9882n_ddi_wr_pack(setpage);
	/*TDI_RD_KEY*/
	ili9882n_ddi_wr_pack(0x1FFF9487);

	/*( *( __IO uint8 *)	(0x4800A) ) =0x2*/
	if (ili9882n_ice_reg_write(touch_dev->dev, 0x4800A, 0x02, 1) < 0)
		TOUCH_E("Write 0x2 at 0x4800A\n");

	*data = ili9882n_ddi_rd_pack(setreg);
	TOUCH_I("check page = 0x%X, reg = 0x%X, read 0x%X\n", page, reg, *data);

	/*TDI_RD_KEY OFF*/
	ili9882n_ddi_wr_pack(0x1FFF9400);
	/*TDI_WR_KEY OFF*/
	ili9882n_ddi_wr_pack(0x1FFF9500);
...
static ssize_t ilitek_proc_read_write_register_read(struct file *pFile, char __user *buf, size_t nCount, loff_t *pos)
{
	...
	if (mcu_status == MCU_ON)
		ret = ili9882n_ice_mode_enable(touch_dev->dev, MCU_ON);
	else
		ret = ili9882n_ice_mode_enable(touch_dev->dev, MCU_STOP);
	if (ret < 0) {
	...
	if (type == REGISTER_READ) {
		ili9882n_ice_reg_read(touch_dev->dev, addr, &read_data, sizeof(read_data));
		TOUCH_I("READ:addr = 0x%06x, read = 0x%08x\n", addr, read_data);
		nCount = snprintf(g_user_buf, PAGE_SIZE, "READ:addr = 0x%06x, read = 0x%08x\n", addr, read_data);

	} else {
		ili9882n_ice_reg_write(touch_dev->dev, addr, write_data, write_len);
	...
	ili9882n_ice_mode_disable(touch_dev->dev);
...
static ssize_t ilitek_proc_dump_flash_write(struct file *filp, const char *buff, size_t size, loff_t *pPos)
{
	...
	ili9882n_dump_flash_data(touch_dev->dev, temp[1], temp[2]);
	return size;
}
...
static ssize_t ilitek_proc_dump_iram_write(struct file *filp, const char *buff, size_t size, loff_t *pPos)
{
	...
	TOUCH_I("Start = 0x%x, End = 0x%x\n", temp[1], temp[2]);
	ili9882n_dump_iram_data(touch_dev->dev, temp[1], temp[2]);
	return size;
}
...
static ssize_t ilitek_proc_fw_upgrade_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	...
	ret = ili9882n_fw_upgrade(touch_dev->dev, fwpath);
	if (ret) {
	...
	ili9882n_reset_ctrl(touch_dev->dev, HW_RESET_SYNC);
...
static long ilitek_proc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case ILITEK_IOCTL_I2C_WRITE_DATA:
		...
		if (res < 0) {
			...
			res = ili9882n_reg_write(touch_dev->dev, szBuf[0], &szBuf[1], (i2c_rw_length-1));
			if (res < 0)
	...
	case ILITEK_IOCTL_I2C_READ_DATA:
		res = ili9882n_reg_read(touch_dev->dev, CMD_NONE, szBuf, i2c_rw_length);
		if (res < 0) {
	...
	case ILITEK_IOCTL_TP_HW_RESET:
		ili9882n_reset_ctrl(touch_dev->dev, HW_RESET_ONLY);
		break;
	...
	case ILITEK_IOCTL_TP_FUNC_MODE:
		...
		if (res < 0) {
			...
			res = ili9882n_reg_write(touch_dev->dev, szBuf[0], &szBuf[1], 2);
		}
		break;
	...
	case ILITEK_IOCTL_TP_DRV_VER:
		...
		ili9882n_tp_info(touch_dev->dev);
		break;
	...
	case ILITEK_IOCTL_TP_MODE_CTRL:
		...
		if (res < 0) {
			...
			ili9882n_switch_fw_mode(touch_dev->dev, szBuf[0]);
		}
		break;
...
static ssize_t ilitek_node_ioctl_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	...
	if (strncmp(cmd, "hwreset", strlen(cmd)) == 0) {
		...
		ili9882n_ic_info(touch_dev->dev);
		...
		ret = ili9882n_reg_write(touch_dev->dev, temp[0], &temp[1], w_len - 1);
		if (ret < 0) {
			TOUCH_E("Write error\n");
		}
		...
		ret = ili9882n_reg_read(touch_dev->dev, CMD_NONE, temp, r_len);
		if (ret < 0)
		...
		ili9882n_get_ddi_reg_onepage(data[1], data[2], &ddi_data);
		...
		ili9882n_set_ddi_reg_onepage(data[1], data[2], data[3]);
		...
		u8 power = 0;
		ili9882n_get_ddi_reg_onepage(0, 0xA, &power);
		TOUCH_I("Check DDI power status = %x\n", power);
	} else if (strncmp(cmd, "pclatch", strlen(cmd)) == 0) {
		ili9882n_get_pc_latch(touch_dev->dev);
	} else {
...
int ili9882n_apk_init(struct device *dev)
{
	int i = 0, res = 0;
	touch_dev = to_ili9882n_data(dev);
