...
static int allnode_open_cdc_data(int mode, int *buf)
{
	...
	if (MP_DBG_MSG)
		ili9882n_dump_packet(cmd, 8, sizeof(cmd), 0, "Open SP command");

	/* NOTE: If TP driver is doing MP test and commanding 0xF1 to FW, we add a checksum
	 * to the last index and plus 1 with size. */
	check_sum = ili9882n_calc_data_checksum(&cmd[0], cdc_len - 1);
	cmd[cdc_len - 1] = check_sum;
	header = cmd[0];
	ret = ili9882n_reg_write(core_mp->dev, header, &cmd[1], cdc_len - 1);
	if (ret < 0) {
	...
	if (core_mp->busy_cdc == POLL_CHECK)
		ret = ili9882n_check_cdc_busy(core_mp->dev, 50, 50);
	else
		ret = ili9882n_check_int_status(core_mp->dev, false);
	...
	if (core_mp->core_ver < CORE_VER_1420) {
		...
		if (ili9882n_reg_write(core_mp->dev, CMD_READ_DATA_CTRL, &cmd[1], 1)) {
		...
		if (ili9882n_reg_write(core_mp->dev, CMD_GET_CDC_DATA, cmd, 0)) {
	...
	if (ili9882n_reg_read(core_mp->dev, CMD_NONE, ori, len)) {
	...
	if (MP_DBG_MSG)
		ili9882n_dump_packet(ori, 8, len, 0, "Open SP CDC original");
	...
	if (MP_DBG_MSG)
		ili9882n_dump_packet(buf, 10, core_mp->frame_len,  core_mp->xch_len, "Open SP CDC combined");
out:
	ipio_kfree((void **)&ori);
	return ret;
}
...
static int allnode_mutual_cdc_data(int index)
{
	...
	if (MP_DBG_MSG)
		ili9882n_dump_packet(cmd, 8, cdc_len, 0, "Mutual CDC command");
	...
	if (MP_DBG_MSG)
		ili9882n_dump_packet(ori, 8, len, 0, "Mutual CDC original");
	...
	if (MP_DBG_MSG)
		ili9882n_dump_packet(frame_buf, 32, core_mp->frame_len, core_mp->xch_len, "Mutual CDC combined");
out:
	ipio_kfree((void **)&ori);
	return ret;
}
...
static int open_sp_test(int index)
{
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		if (MP_DBG_MSG) {
			ili9882n_dump_packet(open[i].cbk_700, 10, core_mp->frame_len, core_mp->xch_len, "cbk 700");
			ili9882n_dump_packet(open[i].cbk_250, 10, core_mp->frame_len, core_mp->xch_len, "cbk 250");
			ili9882n_dump_packet(open[i].cbk_200, 10, core_mp->frame_len, core_mp->xch_len, "cbk 200");
		}
		...
		if (MP_DBG_MSG) {
			ili9882n_dump_packet(open[i].charg_rate, 10, core_mp->frame_len, core_mp->xch_len, "origin charge rate");
			ili9882n_dump_packet(open[i].full_Open, 10, core_mp->frame_len, core_mp->xch_len, "origin full open");
		}
		...
		if (MP_DBG_MSG)
			ili9882n_dump_packet(&tItems[index].buf[(i * core_mp->frame_len)], 10, core_mp->frame_len, core_mp->xch_len, "after full_open_rate_compare");
		...
		if (MP_DBG_MSG)
			ili9882n_dump_packet(&tItems[index].buf[(i * core_mp->frame_len)], 10, core_mp->frame_len, core_mp->xch_len, "after compare charge rate");
...
static int open_c_test(int index)
{
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		if (MP_DBG_MSG)
			ili9882n_dump_packet(open[i].dcl_cap, 10, core_mp->frame_len, core_mp->xch_len, "DCL_Cap");
...
int ili9882n_int_action_ctrl(struct device *dev, bool keep_status)
{
	int ret = 0;
	u8 data[2] = {0};
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	/* Switch to test mode */
	if (ili9882n_switch_fw_mode(dev, FIRMWARE_TEST_MODE)) {
		TOUCH_E("Failed to switch test mode\n");
		goto out;
	}

	if (d->int_action)
		ili9882n_int_action_ctrl(dev, true);
...
out:
	...
	if (d->int_action)
		ili9882n_int_action_ctrl(dev, false);

	/* Reset IC to get back to demo mode.  */
	ili9882n_reset_ctrl(dev, HW_RESET_SYNC);
	/* Switch to demo mode */
	//if (ili9882n_switch_fw_mode(dev, FIRMWARE_DEMO_MODE))
...
static ssize_t show_delta(struct device *dev, char *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9882n_data *d = to_ili9882n_data(dev);
	struct ili9882n_debug_info *debug_info = &d->debug_info;
	struct ili9882n_panel_info *panel_info = &d->tp_info.panel_info;
	int ret = 0, try = 10;
	int log_ret = 0;
...
retry:
	...
	ret = ili9882n_reg_write(dev, CMD_READ_DATA_CTRL, &tmp, 1);

	tmp = 0x01; //Set signal once data mode
	ret = ili9882n_reg_write(dev, 0xB7, &tmp, 1);
	if (ret) {
	...
	ret = ili9882n_reg_read(dev, CMD_NONE, debug_info->data, read_length);
	...
	//Compare checksum
	check_sum = ili9882n_calc_data_checksum(debug_info->data, (read_length - 1));
...
retry:
	...
	if (debug_info->data[0] != 0xB7 && try != 0) {
		...
		ili9882n_dump_packet(debug_info->data, 10, read_length, col, "Header");
		try--;
		goto retry;
	}
	...
	if (check_sum != debug_info->data[read_length -1]) {
		TOUCH_E("Packet check sum Error, check_sum = %d, abs_data = %d\n",
				check_sum, debug_info->data[read_length -1]);
		ili9882n_dump_packet(debug_info->data, 10, read_length, col, "raw");
		try--;
		goto retry;
	}
...
static ssize_t show_noise(struct device *dev, char *buf)
{
	...
	ret = ili9882n_reg_write(dev, CMD_READ_DATA_CTRL, &cmd, 1);
...
retry:
		...
		ret = ili9882n_reg_write(dev, 0xB7, &cmd, 1);
		if (ret) {
			TOUCH_E("Failed to write preparation command, %d\n", ret);
			goto out;
		}
		...
		for (j = 4, index = 0; index < row * col; j += 2, index++) {
			...
			p2p[index] = rawtemp;
			ili9882n_dump_packet(p2p, 16, row * col, col, "p2p raw");
		}
	}

	for (index = 0; index < row * col; index++) {
		p2p[index] = max[index] - min[index];
		ili9882n_dump_packet(max, 16, row * col, col, "max raw");
		ili9882n_dump_packet(min, 16, row * col, col, "min raw");
		ili9882n_dump_packet(p2p, 16, row * col, col, "p2p raw");
	}
...
retry:
		...
		ili9882n_dump_packet(rawdata, 16, row * col, col, "jitter raw");
	}
...
int ili9882n_mp_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
