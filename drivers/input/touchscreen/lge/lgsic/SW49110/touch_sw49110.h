...
#ifndef LGE_TOUCH_SW49110_H
#define LGE_TOUCH_SW49110_H
...
/* device control */
#define TC_VERSION				(0x282)//(0x312)//(0x242)
#define TC_PRODUCT_CODE			(0x283)//(0x313)
#define TC_PRODUCT_ID1			(0x284)//(0x314)//(0x244)
#define TC_PRODUCT_ID2			(0x285)//(0x315)//(0x245)
#define TC_CFG_VERSION			(0x287)//(0x317)
#define IC_STATUS				(0x200)	/* sw49110_touch_info base addr */
...
#define PT_INFO_LCM_TYPE		(0x29C)//(0x32C) /* sw49110_pt_info base addr */
#define PT_INFO_LOT_NUM			(0x29D)//(0x32D)
#define PT_INFO_FPC_TYPE		(0x29E)//(0x32E)
#define PT_INFO_PT_DATE			(0x29F)//(0x32F)
#define PT_INFO_PT_TIME			(0x2A0)//(0x330)
#define PT_INFO_CHIP_REV		(0x2A1)//(0x331)
...
/* Flexible Report */
#define REPORT_PACKET_SIZE		(12) // 12byte = sizeof(sw49110_touch_data)
...
#define spr_code_offset			(0x021) //(0x0AA)
...
enum {
	LGD_INCELL_SW49110 = 0,
	TOVIS_INCELL_SW49110,
};
enum {
	LGD_INCELL_SW49110_HD = 0,
	TOVIS_INCELL_SW49110_HD,
};
#endif
...
struct sw49110_touch_debug_info {
...
struct sw49110_touch_info {
	...
	u32 current_mode:3;
	u32 palm_bit:13;
	struct sw49110_touch_data data[10];	//120byte
} __packed;

struct sw49110_tc_version {
	u8 minor_ver;
	u8 major_ver:4;
	u8 build_ver:4;
	u8 chip_id;
...
struct sw49110_ic_info {
	struct sw49110_tc_version version;
	u8 product_id[9];
	struct sw49110_pt_info pt_info;
};

struct sw49110_data {
	struct device *dev;
	struct kobject kobj;
	struct sw49110_touch_info info;
	struct sw49110_touch_debug_info debug_info;
	struct sw49110_ic_info ic_info;
...
int sw49110_debug_info(struct device *dev);
int sw49110_reset_ctrl(struct device *dev, int ctrl);

static inline struct sw49110_data *to_sw49110_data(struct device *dev)
{
	return (struct sw49110_data *)touch_get_device(to_touch_core(dev));
}

static inline struct sw49110_data *to_sw49110_data_from_kobj(struct kobject *kobj)
{
	return (struct sw49110_data *)container_of(kobj,
			struct sw49110_data, kobj);
}
static inline int sw49110_read_value(struct device *dev,
		u16 addr, u32 *value)
{
	return sw49110_reg_read(dev, addr, value, sizeof(*value));
}

static inline int sw49110_write_value(struct device *dev,
		u16 addr, u32 value)
{
	return sw49110_reg_write(dev, addr, &value, sizeof(value));
}
