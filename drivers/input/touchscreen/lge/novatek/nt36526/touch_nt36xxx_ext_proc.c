...
uint8_t nvt_get_fw_pipe(void)
{
	...
	//TOUCH_I("FW pipe=%d, buf[1]=0x%02X\n", (buf[1]&0x01), buf[1]);
...
void nvt_read_mdata(uint32_t xdata_addr, uint32_t xdata_btn_addr)
{
	...
	for (i = 0; i < ((dummy_len + data_len) / XDATA_SECTOR_SIZE); i++) {
		//---change xdata index---
		nvt_set_page(gts->dev, I2C_FW_Address, head_addr + XDATA_SECTOR_SIZE * i);

		//---read xdata by BUS_TRANSFER_LENGTH
		for (j = 0; j < (XDATA_SECTOR_SIZE / BUS_TRANSFER_LENGTH); j++) {
			//---read data---
			buf[0] = BUS_TRANSFER_LENGTH * j;
			CTP_I2C_READ(gts->dev, I2C_FW_Address, buf, BUS_TRANSFER_LENGTH + 1);
	...
	if (residual_len != 0) {
		//---change xdata index---
		nvt_set_page(gts->dev, I2C_FW_Address, xdata_addr + data_len - residual_len);
	...
	//---change xdata index---
	nvt_set_page(gts->dev, I2C_FW_Address, xdata_btn_addr);

	//---read data---
	buf[0] = (xdata_btn_addr & 0xFF);
	CTP_I2C_READ(gts->dev, I2C_FW_Address, buf, (TOUCH_KEY_NUM * 2 + 1));

	//---2bytes-to-1data---
	for (i = 0; i < TOUCH_KEY_NUM; i++) {
		xdata[d->x_num * d->y_num + i] = (int16_t)(buf[1 + i * 2] + 256 * buf[1 + i * 2 + 1]);
	}
#endif

	//---set xdata index to EVENT BUF ADDR---
	nvt_set_page(gts->dev, I2C_FW_Address, d->mmap->EVENT_BUF_ADDR);
}
...
void nvt_get_mdata(int32_t *buf, uint8_t *m_x_num, uint8_t *m_y_num)
{
    ...
    *m_x_num = d->x_num;
    *m_y_num = d->y_num;
    memcpy(buf, xdata, ((d->x_num * d->y_num + TOUCH_KEY_NUM) * sizeof(int32_t)));
}
...
{
	struct nt36xxx_data *d = to_nt36xxx_data(gts->dev);
	seq_printf(m, "fw_ver=%d, x_num=%d, y_num=%d, button_num=%d\n", d->fw_ver, d->x_num, d->y_num, d->max_button_num);
	return 0;
}
...
static int32_t c_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < d->y_num; i++) {
		for (j = 0; j < d->x_num; j++) {
			seq_printf(m, "%5d, ", xdata[i * d->x_num + j]);
		}
		seq_puts(m, "\n");
	}
...
#if TOUCH_KEY_NUM > 0
	for (i = 0; i < TOUCH_KEY_NUM; i++) {
		seq_printf(m, "%5d, ", xdata[d->x_num * d->y_num + i]);
	}
	seq_puts(m, "\n");
#endif
...
*******************************************************/
int32_t nvt_extra_proc_init(struct device *dev)
{
	...
	if (NVT_proc_fw_version_entry == NULL) {
		TOUCH_E("create proc/%s Failed!\n", NVT_FW_VERSION);
		return -ENOMEM;
	} else {
		TOUCH_I("create proc/%s Succeeded!\n", NVT_FW_VERSION);
	}
