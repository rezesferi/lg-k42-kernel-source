		...
		struct device_attribute *attr, char *buf)
{
	...
	cur_level = regulator_get_voltage(vibr->vibr_conf.reg);
	pr_info("regulator cur_level = %d  \n", cur_level );

	return sprintf(buf, "cur_level = %u, min = %u, max = %u\n", cur_level, vibr->vibr_conf.min_volt, vibr->vibr_conf.max_volt);
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (ret) {
		pr_notice("get level fail!\n");
		return ret;
	}
	vibr = dev_get_drvdata(dev->parent);
	vibr->vibr_conf.min_volt = level;
	vibr->vibr_conf.max_volt = level;
...
static ssize_t vibr_on_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct reg_vibr *vibr = dev_get_drvdata(dev->parent);
	unsigned int enable = 0;

	if (regulator_is_enabled(vibr->vibr_conf.reg)) {
		pr_info("regulator is enabled \n" );
		enable = 1;
	} else {
		pr_info("regulator is disabled \n" );
	}

	return sprintf(buf, "vibr_on state = %d\n", enable);
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (vibr->vibr_conf.reg == NULL) {
		pr_notice("get vib regulater failed!\n");
		return size;
	}
