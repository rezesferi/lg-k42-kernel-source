...
//add for hi556 read otp
#define  HI556_I2C_ADDR 0x50

static void OTP_write_cmos_sensor(unsigned int addr, unsigned int para)
{
	char pu_send_cmd[3] = {(char)(addr >> 8), (char)(addr & 0xFF), (char)(para)};

	iWriteRegI2C(pu_send_cmd, 3, HI556_I2C_ADDR);
}
//end

//add for hi846 read otp
//#ifdef CONFIG_HQ_EH15_CAM
#if 1
#define  HI846_I2C_ADDR 0x40
static void OTP_write_cmos_sensor_HI846(unsigned int addr, unsigned int para)
{
	//char pu_send_cmd[3] = {(char)(addr >> 8), (char)(addr & 0xFF), (char)(para)};
	char pu_send_cmd[4] = {(char)(addr >> 8),
		(char)(addr & 0xFF), (char)(para & 0xFF)};

	iWriteRegI2C(pu_send_cmd, 3, HI846_I2C_ADDR);
}
#endif
//end
/********************************************************************
	...
	(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_pstI2Cclients[I2C_DEV_IDX_1]->addr = 0x51;
...
#endif
{
	...
	case CAM_CALIOC_G_READ:
		...
		//add for hi556 read otp
		if (pcmdInf != NULL && ptempbuf->sensorID == 0x0556) {
			int i = 0;
			u8 *temp = pu1Params;
			pr_info("[hi556]otp read\n");
			OTP_write_cmos_sensor(0x0A02, 0x01);
			OTP_write_cmos_sensor(0x0A00, 0x00);
			mdelay(10);
			OTP_write_cmos_sensor(0x0F02, 0x00);
			OTP_write_cmos_sensor(0x011a, 0x01);
			OTP_write_cmos_sensor(0x011b, 0x09);
			OTP_write_cmos_sensor(0x0d04, 0x01);
			OTP_write_cmos_sensor(0x0d02, 0x07);
			OTP_write_cmos_sensor(0x003e, 0x10);//OTP r/w mode
			OTP_write_cmos_sensor(0x0a00, 0x01);

			OTP_write_cmos_sensor(0x010a, (ptempbuf->u4Offset >> 8) & 0x00FF); //start address H
			OTP_write_cmos_sensor(0x010b, (ptempbuf->u4Offset & 0x00FF)); //start address L
			OTP_write_cmos_sensor(0x0102, 0x01); //read enable
			...
			OTP_write_cmos_sensor(0x0a00, 0x00);
			mdelay(10);
			OTP_write_cmos_sensor(0x003e, 0x00); //complete
			OTP_write_cmos_sensor(0x0a00, 0x01);
		}
//#ifdef CONFIG_HQ_EH15_CAM
#if 1
		else if (pcmdInf != NULL && ptempbuf->sensorID == 0x0846) {
			int i = 0;
			int otp_flag = 1;
			u8 *temp = pu1Params;
			pr_err("[hi846]otp read\n");
			OTP_write_cmos_sensor_HI846(0x0A02, 0x01);
			OTP_write_cmos_sensor_HI846(0x0A00, 0x00);
			mdelay(10);
			OTP_write_cmos_sensor_HI846(0x0F02, 0x00);
			OTP_write_cmos_sensor_HI846(0x071a, 0x01);
			OTP_write_cmos_sensor_HI846(0x071b, 0x09);
			OTP_write_cmos_sensor_HI846(0x0d04, 0x00);
			OTP_write_cmos_sensor_HI846(0x0d00, 0x07);
			OTP_write_cmos_sensor_HI846(0x003e, 0x10);//OTP r/w mode
			OTP_write_cmos_sensor_HI846(0x0a00, 0x01);
            if(ptempbuf->u4Length == 1868 && otp_flag ==1)
            {
				pr_debug("[hi846]otp lsc read\n");
				for(i=0;i<ptempbuf->u4Length;i++)
					pu1Params[i] = OTPData[i];
				otp_flag = 0;
            }
			else if(ptempbuf->u4Length != 1868)
			{
				OTP_write_cmos_sensor_HI846(0x070a, (ptempbuf->u4Offset >> 8) & 0x00FF); //start address H
				OTP_write_cmos_sensor_HI846(0x070b, (ptempbuf->u4Offset & 0x00FF)); //start address L
				OTP_write_cmos_sensor_HI846(0x0702, 0x01); //read enable
				for (i = 0; i< ptempbuf->u4Length; i++) {
					if (pcmdInf->readCMDFunc != NULL)
						i4RetValue =
							pcmdInf->readCMDFunc(pcmdInf->client,
				...
				OTP_write_cmos_sensor_HI846(0x0a00, 0x00);
				mdelay(10);
				OTP_write_cmos_sensor_HI846(0x003e, 0x00); //complete
				OTP_write_cmos_sensor_HI846(0x0a00, 0x01);
			}
		}
#endif
