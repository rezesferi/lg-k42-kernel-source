...
extern void set_pcb_indicator_no_pull(void);
extern void set_pcb_indicator_pull_up(void);
#endif
...
static void md_drdi_gpio_status_scan(void)
{
	...
	CCCI_NORMAL_LOG(0, RPC, "scan didr gpio status\n");
#ifdef CONFIG_LGE_SKU_GPIO_PULL_UP
	...
	for (i = 0; i < ARRAY_SIZE(gpio_mapping_table); i++) {
		...
		if (gpio_id >= 0) {
			...
			CCCI_NORMAL_LOG(0, RPC, "GPIO[%s]%d(%d@md),val:%d\n",
					curr, gpio_id, gpio_md_view, val);
		}
	}
#ifdef CONFIG_LGE_SKU_GPIO_PULL_UP
	set_pcb_indicator_no_pull();
#endif
}
