...
char depth_sensor_name[20] = "(null)";
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	pr_info("[SAT] imgsen check is alive - %s\n", psensor_inst->psensor_name);
...
#define STRINGIZE(stringizedName) TOSTRING(stringizedName)
	...
	for (i = 0; i < MAX_NUM_OF_SUPPORT_SENSOR; i++) {
		pr_debug("[SAT] orderedSearchList[%d]=%d\n",
...
{
    pr_err("show_LGCameraSensorName: rear_camera_name [%s] , front_camera_name [%s] , wide_sensor_name [%s] , depth_sensor_name[%s] , macro_sensor_name[%s] \n", rear_sensor_name, front_sensor_name, wide_sensor_name, depth_sensor_name, macro_sensor_name);

        return sprintf(buf, "FCam:%s^^RCam:%s^^RCam1:%s^^RCam2:%s^^RCam3:%s\n", front_sensor_name, rear_sensor_name, wide_sensor_name, depth_sensor_name, macro_sensor_name);
