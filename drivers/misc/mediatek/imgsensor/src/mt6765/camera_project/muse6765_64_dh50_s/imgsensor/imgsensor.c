	...
	MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	if (psensor_func &&
		...
		if (ret != ERROR_NONE)
			pr_err("[%s] SensorGetInfo failed : %d\n", __func__, ret);
...
/*LGE_CHANGE_S, 2020-04-20, Bringup DH50 SCA, soojong.jin@lge.com*/
	...
	if(!strcmp(g_lge_product_name, "mdh50lm_2mfg")){
		...
		imgsensor_i2c_init(&psensor_inst->i2c_cfg,
			imgsensor_custom_config_sca[(unsigned int)psensor_inst->sensor_idx].i2c_dev);
	}
	else {
	//[LGE_UPDATE_S][bk.bae@lge.com][2020-02-13] revision check for rev11
			if(lge_get_board_revno() == HW_REV_1_1){
				...
				imgsensor_i2c_init(&psensor_inst->i2c_cfg,
				imgsensor_custom_config_reva[(unsigned int)psensor_inst->sensor_idx].i2c_dev);
			} else {
				pr_info("###camera custom i2c other rev");
				imgsensor_i2c_init(&psensor_inst->i2c_cfg,
				imgsensor_custom_config_other_rev[(unsigned int)psensor_inst->sensor_idx].i2c_dev);
			}
