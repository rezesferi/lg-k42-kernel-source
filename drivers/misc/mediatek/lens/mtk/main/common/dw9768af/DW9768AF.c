...
static void sCheckVBUSY(void)
{
    ...
    while(1)
    {
        ...
        mdelay(5);
        busy_count++;
        LOG_INF("[DW9768AF] I2C is busy now wait 5ms : busy_count=%d\n", busy_count);
        if(busy_count > 10)
            break;
    }
}
...
{
    int i4RetValue = 0;
    char puSendCmd[3] = { 0x03, (char)((a_u2Data >> 8) & 0xFF), (char)(a_u2Data & 0xFF) };
...
/* ////////////////////////////////////////////////////////////// */
long DW9768AF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command,
...
{
    char puSendCmd1[2] = {0x0A, 0x7D};    //preset position 250code
    char puSendCmd2[2] = {0x0C, 0xA5};    //NRC step delay time 179us
    char puSendCmd3[2] = {0x0B, 0x01};    //NRC enable
    //char puSendCmd4[2] = {0x02, 0x01};  //power down mode /* LGE_CHANGE, 2020-03-19, fixed tick noise issue, ssora.lee@lge.com */

    LOG_INF("Start\n");
    if (*g_pAF_Opened == 2) {
        LOG_INF("Wait : g_u4CurrPosition=%d\n", g_u4CurrPosition);

        moveAF(250);
        msleep(10);
        LOG_INF("Wait : after moveAF(250)\n");
        ...
        msleep(20);
        //i2c_master_send(g_pstAF_I2Cclient, puSendCmd4, 2);    //Power down mode /* LGE_CHANGE, 2020-03-19, fixed tick noise issue, ssora.lee@lge.com */
    }
...
int DW9768AF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
