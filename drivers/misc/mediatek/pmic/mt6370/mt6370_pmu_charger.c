...
#ifdef CONFIG_LGE_PM
static int mt6370_is_hz_enabled(struct mt6370_pmu_charger_data *chg_data,
				bool *en)
{
	return mt6370_pmu_reg_test_bit(chg_data->chip, MT6370_PMU_REG_CHGCTRL1,
...
static int mt6370_enable_chip(struct charger_device *chg_dev, bool en)
{
	...
	/* en is opposite value of hz_en */
	en = en ? false : true;

	ret = mt6370_is_hz_enabled(chg_data, &hz_en);
	if (ret >= 0 && hz_en == en)
		return 0;

	ret = mt6370_enable_hz(chg_data, en);
	if (ret < 0) {
		dev_err(chg_data->dev, "%s: %s hz failed\n", __func__,
			en ? "enable" : "disable");
	}
...
static int mt6370_enable_te(struct charger_device *chg_dev, bool en)
{
	struct mt6370_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);
#ifdef CONFIG_LGE_PM
	struct mt6370_pmu_chip *chip = chg_data->chip;

	chip->is_te_enabled = en;
#endif
...
static int __mt6370_is_power_path_enable(struct mt6370_pmu_charger_data *chg_data, bool *en)
{
...
#ifdef CONFIG_LGE_USB_TYPE_C
	return __mt6370_is_power_path_enable(chg_data, en);
#else
	mutex_lock(&chg_data->pp_lock);
	*en = chg_data->pp_en;
	mutex_unlock(&chg_data->pp_lock);
...
#ifdef CONFIG_LGE_PM
	charger_dev_notify(chg_data->chg_dev, CHARGER_DEV_NOTIFY_MIVR);
#endif
	return IRQ_HANDLED;
}
...
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
static void chgctrl_hook(struct mt6370_pmu_charger_data *chg_data,
			 enum power_supply_property psp,
			 union power_supply_propval *val)
{
	struct chgctrl_helper *helper;

	if (IS_ERR_OR_NULL(chg_data->chgctrl_psy)) {
		chg_data->chgctrl_psy =
			devm_power_supply_get_by_phandle(chg_data->dev,
				"charger-controller");
	}
	if (IS_ERR_OR_NULL(chg_data->chgctrl_psy))
		return;

	helper = power_supply_get_drvdata(chg_data->chgctrl_psy);
	if (!helper || !helper->charger_hook)
		return;

	helper->charger_hook(helper, psp, val);
}
#endif
...
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
static bool floated_check = false;
#endif
...
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
	if ((memcmp(chg_data->old_propval, propval, sizeof(propval))) || floated_check) {
...
#ifdef CONFIG_TCPC_CLASS
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
static void chgctrl_set_bc12_type(struct mt6370_pmu_charger_data *chg_data,
				  ...
				  enum power_supply_usb_type usb_type)
{
	...
	if (type == 5 && usb_type == 0) {
		floated_check = true;
		pr_info("%s floated_detect", __func__);
	}
...
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
	chgctrl_set_bc12_type(chg_data, chg_data->psy_desc.type,
				...
				unsigned long event, void *data)
{
	...
	#ifdef CONFIG_LGE_USB_TYPE_C
	bool is_power_path_enabled = false;
	int ret = 0;
	#endif
	struct mt6370_pmu_charger_data *chg_data =
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (old_state == TYPEC_UNATTACHED &&
			...
			/* Debug accesssory plug in */
			pr_info("%s debug accessory plug in\n", __func__);

			ret = __mt6370_is_power_path_enable(chg_data,
							&is_power_path_enabled);
			if (ret < 0) {
				pr_err("get power path enable fail\n");
				break;
			}

			if (is_power_path_enabled) {
				chg_data->typec_attach = 1;
				chg_data->psy_desc.type =
						POWER_SUPPLY_TYPE_USB;
				chg_data->psy_usb_type =
						POWER_SUPPLY_USB_TYPE_SDP;
				power_supply_changed(chg_data->psy);
			} else {
				chg_data->typec_attach = 0;
				chg_data->psy_desc.type =
						POWER_SUPPLY_TYPE_UNKNOWN;
				chg_data->psy_usb_type =
						POWER_SUPPLY_USB_TYPE_UNKNOWN;
			...
			/* Debug accessory plug out */
			pr_info("%s debug accessory plug out\n", __func__);
...
static void mt6370_pmu_charger_shutdown(struct platform_device *pdev)
{
	struct mt6370_pmu_charger_data *chg_data = platform_get_drvdata(pdev);
	struct mt6370_pmu_chip *chip = chg_data->chip;
	const char magic[] = "5526789";

	if (!chip->ship_mode)
		return;

	shipping_mode_store(&pdev->dev,
			(struct device_attribute *)&dev_attr_shipping_mode,
			magic, strlen(magic));
}
#endif
