...
#ifdef CONFIG_LGE_PM
	/* reset all fled/ldo/rgb/bl/db reg and logic, without chg when no battery */
	ret = mt6370_pmu_reg_write(core_data->chip,
		MT6370_PMU_REG_CORECTRL2,
		core_data->chip->is_te_enabled ? 0x7F : 0x3F);
#else
...
static struct mt6370_pmu_core_data *syscore_core_data = NULL;
...
static void mt6370_pmu_core_syscore_shutdown(void)
{
	int ret = 0;

	if (!syscore_core_data) {
		pr_err("%s: is NULL\n", __func__);
		return;
	}

	ret = mt6370_pmu_core_reset(syscore_core_data);
	if (ret < 0)
		dev_err(syscore_core_data->dev, "pmu core reset fail\n");
}

struct syscore_ops mt6370_pmu_core_syscore_ops = {
	.shutdown = mt6370_pmu_core_syscore_shutdown,
};

static int __init mt6370_pmu_core_init(void)
{
	platform_driver_register(&mt6370_pmu_core);
	register_syscore_ops(&mt6370_pmu_core_syscore_ops);
	return 0;
}

static void __exit mt6370_pmu_core_exit(void)
{
	unregister_syscore_ops(&mt6370_pmu_core_syscore_ops);
	platform_driver_unregister(&mt6370_pmu_core);
}

module_init(mt6370_pmu_core_init);
module_exit(mt6370_pmu_core_exit);
