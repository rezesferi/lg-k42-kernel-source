...
#ifdef CONFIG_LGE_SENSOR_CUSTOM_CALI
		if (!data_buf[0] && !data_buf[1] && !data_buf[2]) {
			err = sensor_calibration_read(ID_ACCELEROMETER, data_buf);
			if (err) {
				pr_err("GSENSOR_IOCTL_SET_CALI sensor_calibration_read fail\n");
				return -EINVAL;
			}
		} else {
			err = sensor_calibration_save(ID_ACCELEROMETER, data_buf);
			if (err) {
				pr_err("GSENSOR_IOCTL_SET_CALI sensor_calibration_save fail\n");
				return -EINVAL;
			}
		}
#endif
