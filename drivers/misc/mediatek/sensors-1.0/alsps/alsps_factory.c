...
#ifdef CONFIG_LGE_SENSOR_CUSTOM_CALI
					 ...
					 unsigned long arg)
{
	...
	case ALSPS_GET_PS_MODE:
		if (alsps_factory.fops && alsps_factory.fops->ps_get_enable) {
			err = alsps_factory.fops->ps_get_enable(&data);
			if (err < 0) {
				pr_err("ALSPS_GET_PS_MODE fail!\n");
				return -EINVAL;
			}
			pr_info("ALSPS_GET_PS_MODE data: %d", data);
			if (copy_to_user(ptr, &data, sizeof(data)))
				return -EFAULT;
		} else {
			pr_err("ALSPS_GET_PS_MODE NULL\n");
			return -EINVAL;
		}
		return 0;
#endif
...
#ifdef CONFIG_LGE_SENSOR_PS_OFFSET_CALI
			pr_info("ALSPS_GET_PS_RAW_DATA: ps_offset = %d 0x%x\n", data >> 16, data);
			data &= 0xffff;
#endif
			if (copy_to_user(ptr, &data, sizeof(data)))
				return -EFAULT;
	...
	case ALSPS_IOCTL_SET_CALI:
		...
		if (data == 0xFFFF) {
			...
			if (err) {
				pr_err("ALSPS_IOCTL_SET_CALI sensor_calibration_read fail\n");
				return -EINVAL;
			}
		} else {
			err = sensor_calibration_save(ID_PROXIMITY, &data);
			if (err) {
				pr_err("ALSPS_IOCTL_SET_CALI sensor_calibration_save fail\n");
				return -EINVAL;
			}
		}
		pr_info("ALSPS_IOCTL_SET_CALI: %d\n", data);
#endif
		if (alsps_factory.fops != NULL &&
...
#ifdef CONFIG_LGE_SENSOR_PS_OFFSET_CALI
			if (alsps_factory.fops->ps_get_raw_data) {
				err = alsps_factory.fops->ps_get_raw_data(&data);
				if (err < 0) {
					pr_err("ALSPS_IOCTL_SET_CALI ps_get_raw_data fail!\n");
					return -EINVAL;
				}
				pr_info("ALSPS_IOCTL_SET_CALI: ps_offset = %d 0x%x\n", data >> 16, data);
			}
#endif
		} else {
