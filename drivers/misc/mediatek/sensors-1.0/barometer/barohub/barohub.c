...
static int barohub_factory_clear_cali(void)
{
	...
	obj->baro_cali = 0;
#ifdef MTK_OLD_FACTORY_CALIBRATION
	err = sensor_set_cmd_to_hub(ID_PRESSURE, CUST_ACTION_RESET_CALI, &obj->baro_cali);
	if (err < 0) {
		pr_err( "sensor_set_cmd_to_hub fail, (ID: %d),(action: %d)\n",
			ID_PRESSURE, CUST_ACTION_RESET_CALI);
			return -1;
	}
#endif
#endif
...
static int barohub_factory_set_cali(int32_t offset)
{
	...
	obj->baro_cali = offset;
#ifdef CONFIG_LGE_SENSOR_CUSTOM_CALI
	err = sensor_set_cmd_to_hub(ID_PRESSURE,
			CUST_ACTION_SET_CALI, &obj->baro_cali);
	if (err < 0) {
		pr_err("sensor_set_cmd_to_hub fail, (ID: %d),(action: %d)\n",
