					...
					unsigned long arg)
{
	...
	case BAROMETER_GET_TEMP_DATA:
		return 0;
#ifdef CONFIG_LGE_SENSOR_BARO_CALI
	case BAROMETER_IOCTL_CLR_CALI:
		if (baro_factory.fops != NULL &&
		    baro_factory.fops->clear_cali != NULL) {
			err = baro_factory.fops->clear_cali();
			if (err < 0) {
				pr_err("BAROMETER_IOCTL_CLR_CALI FAIL!\n");
				return -EINVAL;
			}
		} else {
			pr_err("BAROMETER_IOCTL_CLR_CALI NULL\n");
			return -EINVAL;
		}
		return 0;
...
#ifdef CONFIG_LGE_SENSOR_CUSTOM_CALI
		if (data == 0xffff) {
			err = sensor_calibration_read(ID_PRESSURE, &data);
			if (err) {
				pr_err("BAROMETER_IOCTL_SET_CALI sensor_calibration_read fail\n");
				return -EINVAL;
			}
		} else {
			err = sensor_calibration_save(ID_PRESSURE, &data);
			if (err) {
				pr_err("BAROMETER_IOCTL_SET_CALI sensor_calibration_save fail\n");
				return -EINVAL;
			}
		}
#endif
		printk("BAROMETER_IOCTL_SET_CALI : %d\n", data);
		if (baro_factory.fops != NULL &&
				baro_factory.fops->set_cali != NULL) {
			err = baro_factory.fops->set_cali(data);
			if (err < 0) {
