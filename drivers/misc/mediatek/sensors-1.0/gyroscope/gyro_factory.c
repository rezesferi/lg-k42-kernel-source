					...
					unsigned long arg)
{
	...
	case GYROSCOPE_IOCTL_WRITE_CALI:
		...
		if (err) {
			pr_err("GSENSOR_IOCTL_WRITE_CALI sensor_calibration_save fail\n");
			return -EINVAL;
		}
		pr_notice("GYROSCOPE_IOCTL_WRITE_CALI: (%d, %d, %d)!\n",
			 data_buf[0], data_buf[1], data_buf[2]);
		if (gyro_factory.fops && gyro_factory.fops->set_cali) {
	...
	case GYROSCOPE_IOCTL_SET_CALI:
		...
		if (err) {
			pr_err("GYROSCOPE_IOCTL_SET_CALI sensor_calibration_read fail\n");
			return -EINVAL;
		}
#endif
