...
static int get_dt_mboot_boottype(void)
{
	...
	tag = (struct tag_bootmode *)of_get_property(np_chosen, "atag,boot",
						    NULL);
	if (!tag) {
		pr_info("[%s] %s: fail to get atag,boot\n", TC1_MOD, __func__);
	} else {
		pr_debug("[%s] bootmode: 0x%x boottype: 0x%x\n", TC1_MOD,
...
static int get_misc2_path(void)
{
	...
	snprintf(misc2_path, sizeof(misc2_path),
		"/dev/block/by-name/%s", PART_NAME);

	boottype = get_dt_mboot_boottype();
	if (boottype == 2)
		g_block_size = 4096;

	pr_debug("[%s] misc2 path : %s, blocksize[%d]\n", TC1_MOD, misc2_path, g_block_size);
