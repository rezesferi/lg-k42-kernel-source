...
pr_debug("[Thermal/TZ/QUIET]", ##args)
...
static const struct file_operations lgets_QUIET_fops = {
	.owner = THIS_MODULE,
...
static const struct file_operations lgets_QUIET_param_fops = {
	.owner = THIS_MODULE,
...
static int lgets_quiet_probe(struct platform_device *pdev)
{
	...
	struct proc_dir_entry *lgets_QUIET_dir = NULL;
	...
	if (ret) {
		lgets_quiet_printk("[%s] fail to get auxadc iio ch2: %d\n",
			__func__, ret);
		return ret;
	}
	...
	lgets_QUIET_dir = mtk_thermal_get_proc_drv_therm_dir_entry();
	if (!lgets_QUIET_dir) {
		lgets_quiet_dprintk(
			"[%s]: mkdir /proc/driver/thermal failed\n",
				__func__);
	} else {
		entry = proc_create("tzquiet", 0664, lgets_QUIET_dir,
							&lgets_QUIET_fops);
		if (entry)
			proc_set_user(entry, uid, gid);

		entry = proc_create("tzquiet_param", 0664, lgets_QUIET_dir,
				&lgets_QUIET_param_fops);
		if (entry)
			proc_set_user(entry, uid, gid);
