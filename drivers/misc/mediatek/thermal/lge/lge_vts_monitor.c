...
static bool vts_ext_supply = false;
...
static void vts_cmode_update(struct work_struct *work)
{
	...
	if (!vts_display_on && vts_ext_supply)
		cmode = true;
					 ...
					 unsigned long event, void *v)
{
	...
	ext_supply = power_supply_is_system_supplied() > 0 ? true : false;

	if (ext_supply == vts_ext_supply)
		return NOTIFY_DONE;

	vts_ext_supply = ext_supply;
	schedule_delayed_work(&vts_cmode, 0);
...
/* monitor */
static int vts_monitor_interval = (30 * 1000);	/* 30 seconds */
...
static void vts_monitor_info(struct work_struct *work)
{
	...
	int mdpa = vts_monitor_get_temp(MTK_THERMAL_SENSOR_MD_PA);

	pr_info("CPU( %d ) BOARD( %d ) BATTRY( %d ) QUIET( %d ) SKIN( %d )"
		" PA ( %d )\n",
		cpu, board, battery, quiet, skin, mdpa);
