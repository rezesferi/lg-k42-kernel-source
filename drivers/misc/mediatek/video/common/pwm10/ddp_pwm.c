...
#ifdef CONFIG_LGE_DISPLAY_COMMON
	bl_on_delay = primary_display_get_pwm_on_delay();
	if (bl_on_delay && g_pwm_led_mode == MT65XX_LED_MODE_CUST_BLS_PWM &&
			atomic_read(&g_pwm_is_power_on[index]) == 0 && level_1024 > 0) {
		PWM_ERR("wait bl_on_delay : %d msec", bl_on_delay);
		msleep(bl_on_delay);
/*
		int i;
		for (i = 0; i < wait_vsync; i++) {
			PWM_ERR("wait vsync %d", i);
			dpmgr_wait_event_timeout(primary_get_dpmgr_handle(),
			DISP_PATH_EVENT_IF_VSYNC, HZ / 2);
		}
*/
	}
#endif
