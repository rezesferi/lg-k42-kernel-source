...
struct display_primary_path_context *lge_pgc;
static int esd_on_flag = 0;
#endif
...
#if defined(CONFIG_LGE_DSI_PARAM_TUNING)
extern uint32_t PanelMaster_get_dsi_timing(uint32_t dsi_index, enum MIPI_SETTING_TYPE type);
extern void PanelMaster_DSI_set_timing(uint32_t dsi_index, struct MIPI_TIMING timing);
extern void *primary_get_pgc(void);
#endif

#if defined(CONFIG_LGE_BACKLIGHT_BRIGHTNESS_TUNING)
extern unsigned int *blmap_arr;
extern unsigned int blmap_size;
#if defined(CONFIG_LGE_LCM_BACKLIGHT_PWM)
extern void set_pwm_backlight_brightness(int level_1024);
extern int get_pwm_backlight_brightness(void);
#else
