...
static int mmc_host_set_ffu(struct mmc_card *card, u32 ffu_enable)
{
	...
	if (FFU_ENABLED(ffu_enable)) {
		...
		if (err) {
			pr_info("%s: switch to FFU failed with error %d\n",
...
int mmc_ffu_install(struct mmc_card *card, u8 *ext_csd)
{
	...
	if (FFU_FEATURES(ext_csd[EXT_CSD_FFU_FEATURES])) {
		...
		if (timeout == 0 || timeout > 0x17) {
			timeout = 0x17;
			pr_info("FFU: %s: operation code timeout is out of range. Using maximum timeout.\n",
				mmc_hostname(card->host));
		}
		...
		if (err) {
			pr_info("FFU: %s: error %d setting install mode\n",
				mmc_hostname(card->host), err);
			mmc_ffu_restart(card);
			return err;
		}
	...
	while (retry--) {
		...
		if (err)
			pr_info("FFU: %s: sending ext_csd retry times %d\n",
				mmc_hostname(card->host), retry);
		else
			break;
	}
	if (err) {
		pr_info("FFU: %s: sending ext_csd error %d\n",
			mmc_hostname(card->host), err);
		goto exit;
	}
	...
	if (!err) {
		pr_info("FFU: %s: succeed FFU\n",
			mmc_hostname(card->host));
	} else if (err) {
		pr_info("FFU: %s: error %d FFU install:\n",
			mmc_hostname(card->host), err);
		err = -EINVAL;
	}
...
void mmc_wait_for_ffu_req(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	if (card->ext_csd.cmdq_en && mmc_cmdq_disable(card)) {
		pr_info("%s: disable cmdq fail\n",__func__);
		return;
	}
	...
	if (err) {
		pr_info("FFU: %s: error %d sending ext_csd\n",
			mmc_hostname(card->host), err);
		goto exit;
	}
	...
	if (!FFU_SUPPORTED_MODE(ext_csd[EXT_CSD_SUPPORTED_MODE])) {
		err = -EINVAL;
		pr_info("FFU: %s: error %d FFU is not supported\n",
			mmc_hostname(card->host), err);
		goto exit;
	}
	...
	if (cmd->opcode == MMC_FFU_DOWNLOAD_OP
		...
		if (err) {
			pr_info("FFU: %s: error %d FFU fails to enable\n",
					mmc_hostname(card->host), err);
			err = -EINVAL;
			goto exit;
		}
		...
		if (err) {
			pr_info("FFU: %s: error %d when switch to FFU\n",
				mmc_hostname(card->host), err);
			err = -EINVAL;
			goto exit;
		}
	...
	if (cmd->opcode == MMC_FFU_DOWNLOAD_OP){
		...
		*/
		...
		if (err) {
			pr_info("FFU: %s: error %d write fail\n",
					mmc_hostname(card->host), err);
			goto exit;
		}

		pr_info("FFU:  write success\n");
...
exit:
	...
	if (!card->ext_csd.cmdq_en && mmc_cmdq_enable(card)) {
		pr_info("%s: enable cmdq fail\n",__func__);
		return;
	}
	return;
}
