#define pr_fmt(fmt) "[CHGCTRL-OTP]%s: " fmt, __func__
...
static int chgctrl_otp_get_present(struct chgctrl_otp *otp)
{
...
static int chgctrl_otp_run(struct chgctrl *chip, int event)
{
	...
	if (event & ALGO_EVENT_BATTERY) {
		if (!chgctrl_otp_get_present(otp)) {
			cancel_work_sync(&otp->work);

			chgctrl_vote(chip->fcc, FCC_VOTER_OTP, -1);
			chgctrl_vote(chip->vfloat, VFLOAT_VOTER_OTP, -1);
...
struct chgctrl_algo chgctrl_algo_otp = {
	.name = "otp",
