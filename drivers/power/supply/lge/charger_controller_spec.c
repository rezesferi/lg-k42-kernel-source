#define pr_fmt(fmt) "[CHGCTRL-SPEC]%s: " fmt, __func__
...
static int chgctrl_spec_get_present(struct chgctrl_spec *spec)
{
	struct chgctrl *chip = container_of(spec, struct chgctrl, spec);

	return chip->info.present;
}

static int chgctrl_spec_get_voltage(struct chgctrl_spec *spec)
{
	struct chgctrl *chip = container_of(spec, struct chgctrl, spec);

	return chip->hook.voltage_now;
}

static int chgctrl_spec_get_temp(struct chgctrl_spec *spec)
{
	struct chgctrl *chip = container_of(spec, struct chgctrl, spec);

	return chip->hook.temp;
}
...
static void chgctrl_spec_vote_work(struct work_struct *work)
{
	...
	/* not charging. update immediately */
	if (chip->info.status == POWER_SUPPLY_STATUS_DISCHARGING) {
		fcc = target_fcc;
		goto vote;
	}
	...
	volt = chgctrl_spec_get_voltage(spec) / 1000;
	if (volt > spec->vfloat)
		volt = spec->vfloat;
...
static void chgctrl_spec_work(struct work_struct *work)
{
	...
	/* update battery data */
	volt = chgctrl_spec_get_voltage(spec) / 1000;
	temp = chgctrl_spec_get_temp(spec) / 10;
...
static int chgctrl_spec_run(struct chgctrl *chip, int event)
{
	...
	if (event & ALGO_EVENT_BATTERY) {
		if (!chgctrl_spec_get_present(spec)) {
			cancel_work_sync(&spec->work);
			cancel_delayed_work_sync(&spec->vote_work);

			chgctrl_vote(chip->fcc, FCC_VOTER_SPEC, -1);
