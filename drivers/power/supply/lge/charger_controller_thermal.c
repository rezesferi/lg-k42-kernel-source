#define pr_fmt(fmt) "[CHGCTRL-THERMAL]%s: " fmt, __func__
...
static int chgctrl_thermal_run(struct chgctrl *chip, int event)
{
...
struct chgctrl_algo chgctrl_algo_thermal = {
	.name = "thermal",
	.parse_dt = chgctrl_thermal_parse_dt,
	.init = chgctrl_thermal_init,
	.run = chgctrl_thermal_run,
};
