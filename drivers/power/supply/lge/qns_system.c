/*
 * qns_system.c version 2.1
 * Qnovo QNS wrapper implementation. Compatible with kernel 4.19.
...
static struct power_supply* mt6370_charger_psy = NULL;
static int c = 0, v = 0;
...
static struct wakeup_source *wakelock = NULL;
static bool wakelock_inited = false;
static bool wakelock_held = false;

static struct wakeup_source *charge_wakelock = NULL;
...
static struct power_supply* qns_get_mt6370_charger_psy(const char *err_msg)
{
	if (mt6370_charger_psy)
		return mt6370_charger_psy;

	mt6370_charger_psy = power_supply_get_by_name("mt6370_pmu_charger");
	if (mt6370_charger_psy == NULL)
		pr_err("QNS: ERROR: unable to get mt6370_pmu_charger. %s\n", err_msg);
...
static int qns_set_ibat(int mA)
{
	...
	if (!helper || !helper->set_qns_fcc){
		pr_err("QNS: ERROR: helper->set_qns_fcc\n");
		return QNS_ERROR;
	}

	helper->set_qns_fcc(helper, mA);
	prev_mA = mA;
...
static int qns_set_vbat(int mV)
{
	...
	helper = power_supply_get_drvdata(psy);
	if (!helper || !helper->set_qns_vfloat){
		pr_err("QNS: ERROR: helper->set_qns_vfloat\n");
		return QNS_ERROR;
	}

	helper->set_qns_vfloat(helper, mV);
	prev_mV = mV;
...
static bool qns_is_charging(void)
{
	...
	psy = qns_get_mt6370_charger_psy("Can't read mt6370_charging state!");
	if (!psy)
		return false;

	if (power_supply_get_property(psy, POWER_SUPPLY_PROP_STATUS, &propVal) < 0) {
		pr_err("QNS: ERROR: unable to read mt6370_charger properties! Dose %s have "
