...
int get_efuse_partition_data(unsigned char *result)
{
	...
	curr_fs = get_fs();
	set_fs(KERNEL_DS);
	ret = vfs_read(filp, result, EFUSE_PARTITION_USED_SIZE, &(filp->f_pos));
...
u32 read_efuse_partition(void)
{
	...
	ahash_request_set_crypt(req, &sg, hash_p, sizeof(efuse_partition_all));
...
{
	int i;
	u32 value_part = 0, value_memory = 0, fuse_value = 0;
	u32 temp_same = 0, temp_mtk = 0, temp_lge = 0, temp_jdm = 0, temp_trf =0, temp_dummy = 0;
...
// ---------------------------------
	...
	for (i = 0; i < ARRAY_SIZE(pubk_list); i++) {
		...
		if (value_part == pubk_list[i].lge_default_key)
			temp_lge |= (0x1 << i);
		if (value_part == pubk_list[i].mtk_jdm || value_part == pubk_list[i].lsms_jdm)
			temp_jdm |= (0x1 << i);
		if (value_part == pubk_list[i].lsms_trf)
			temp_trf |= (0x1 << i);
		if (value_part == pubk_list[i].dummy)
			temp_dummy |= (0x1 << i);
	}
	// dummy partition
	if (temp_dummy == 0xFF)
	...
	// jdm key
	printk(KERN_INFO "[EFUSE]%s: check jdmk\n", __func__);
	if (temp_jdm == 0xFF)
		*result_part |= (0x1 << EFUSE_RESULT_CUSTOM_PUB_KEY);
	if (temp_jdm == 0xFF && temp_same == 0xFF)
		*result_verify |= (0x1 << EFUSE_RESULT_CUSTOM_PUB_KEY);
	printk(KERN_INFO "[EFUSE]%s: Current result (verification,result) = (%x, %x)\n", __func__, *result_verify, *result_part);

	// trf key
	printk(KERN_INFO "[EFUSE]%s: check trfk\n", __func__);
	if (temp_trf == 0xFF)
		*result_part |= (0x1 << EFUSE_RESULT_COMMON_CTRL);
	if (temp_trf == 0xFF && temp_same == 0xFF)
		*result_verify |= (0x1 << EFUSE_RESULT_COMMON_CTRL);
	printk(KERN_INFO "[EFUSE]%s: Current result (verification,result) = (%x, %x)\n", __func__, *result_verify, *result_part);
	...
	// set the 0x1 bit MT6762 can read only 3bit
	if ( *result_part == 0x1E || *result_part == 0x3E || *result_part == 0x5E) {
		*result_part |= 0x1;
		fuse_value = read_efuse_memory(SBC_KERNEL_INDEX);
		fuse_value &= SBC_KERNEL_MASKING;
		if ( fuse_value > 0 && (*result_verify == 0x1E || *result_verify == 0x3E  || *result_verify == 0x5E))
			*result_verify |= 0x1;
	}
	printk(KERN_INFO "[EFUSE]%s: Final result (verification,result) = (%x, %x)\n", __func__, *result_verify, *result_part);
...
static ssize_t efuse_read_version_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	m1 = read_efuse_memory(IDX_SECURE_GROUP);
	m2 = read_efuse_memory(IDX_NON_SECURE_GROUP);
	...
	if (strcmp(attr->attr.name, "modem") == 0) {
		return sprintf(buf, "%d\n", v2);
	}
...
static DEVICE_ATTR(modem, S_IWUSR | S_IRUGO, efuse_read_version_show, NULL);
