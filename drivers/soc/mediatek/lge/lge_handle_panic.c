...
static struct task_struct *wdog_tsk[8];
extern int get_HW_cpuid(void);
static int kwdt_thread_wdog(void *arg)
{
...
static int gen_wdog(const char *val, const struct kernel_param *kp)
{
	...
	for (i = 0; i < nr_cpu_ids; i++) {
		sprintf(name, "gen-wdog-%d", i);
		pr_notice("[WDOG]thread name: %s\n", name);
		wdog_tsk[i] = kthread_create(kwdt_thread_wdog, NULL, name);
		if (IS_ERR(wdog_tsk[i])) {
			int ret = PTR_ERR(wdog_tsk[i]);
			wdog_tsk[i] = NULL;
			return ret;
		}
		kthread_bind(wdog_tsk[i], i);
	}

	for (i = 0; i < nr_cpu_ids; i++) {
		pr_notice(" wake_up_process(wdog_tsk[%d])\n", i);
		wake_up_process(wdog_tsk[i]);
	}
...
#ifdef CONFIG_LGE_DRAM_CONSOLE
static u32 dramconsole_address = 0;
static u32 dramconsole_size = 0;
#endif
...
#ifdef CONFIG_LGE_DRAM_CONSOLE
static int lge_get_reserved_dramconsole(struct reserved_mem *rmem)
{
	dramconsole_address = (u32) rmem->base;
	dramconsole_size = (u32) rmem->size;
...
#ifdef CONFIG_LGE_DRAM_CONSOLE
RESERVEDMEM_OF_DECLARE(lge_reserved_dramconsole_init,
		"lge,dramconsole", lge_get_reserved_dramconsole);
#endif
...
static void release_reserved_mem(void)
{
	...
	if (lk_mem_addr != 0 && lk_mem_size != 0) {
...
#ifdef CONFIG_LGE_DRAM_CONSOLE
	buffer = memremap(start, size, MEMREMAP_WB);
#else
