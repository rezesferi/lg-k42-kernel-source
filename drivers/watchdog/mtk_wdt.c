...
#if (USE_MPCORE_WDT_PING)
static DEFINE_SPINLOCK(lock);
static unsigned long kick_bit;

void kick_ex_wdt(struct watchdog_device *wdt_dev)
{
...
static void  mtk_wdt_ping_mpcore(void* data)
{
	int cpu;
	unsigned long flags;
	unsigned long online_bit=0;
	...
	pr_err("kick_bit=0x%lX, online=0x%lx, num_cpu=%u\n", kick_bit, online_bit, num_present_cpus());
	if(online_bit == kick_bit) {
		kick_ex_wdt((struct watchdog_device*)data);
		kick_bit=0;
	}
...
#else
	pr_err("mtk_wdt_ping\n");
	on_each_cpu(mtk_wdt_ping_mpcore, wdt_dev, 0);
#endif
