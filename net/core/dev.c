	...
	if ((sysctl_lge_carrier == 45006 || sysctl_lge_carrier == 45008) && (strncmp(dev->name, "ccmni", 5) == 0)) {
		...
		if (in6_dev) {
			...
			if ((in6_dev->if_flags == RXED_MTU) && (in6_dev->cnf.mtu6 >= IPV6_MIN_MTU)) {
				printk(" 1) dev_set_mtu orig_mtu %d, new_mtu %d change new mtu %d,\n",orig_mtu, new_mtu,in6_dev->cnf.mtu6);
				new_mtu = in6_dev->cnf.mtu6;
			}
			if ((in6_dev->if_flags == RXED_MTU) && (in6_dev->cnf.ra_mtu >= IPV6_MIN_MTU)) {
				printk(" 2) dev_set_mtu orig_mtu %d, new_mtu %d change new mtu %d,\n",orig_mtu, new_mtu,in6_dev->cnf.ra_mtu);
				new_mtu = in6_dev->cnf.ra_mtu;
			}
