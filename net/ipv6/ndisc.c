	...
	if ( (sysctl_lge_carrier == LGU_CARRIER || sysctl_lge_carrier == KT_CARRIER) && (strncmp(skb->dev->name, "ccmni", 5) == 0) ) {
		if (ndopts.nd_opts_mtu) {
			...
			if (mtu < IPV6_MIN_MTU) {
				...
				if (rt) {
					fib6_metric_set(rt, RTAX_MTU, in6_dev->cnf.mtu6);
				}
				rt6_mtu_change(skb->dev, in6_dev->cnf.mtu6);
			} else {

				//pr_err("[mtk_net][%s] ra msg has mtu:%d, dev->mtu:%d, ra_mtu:%d\n",
				//	__func__, mtu, skb->dev->mtu, in6_dev->cnf.ra_mtu);
				if (in6_dev->cnf.ra_mtu != mtu) {
					in6_dev->cnf.ra_mtu = mtu;
					pr_err("[mtk_net]update ra_mtu to %d\n", in6_dev->cnf.ra_mtu);
				}
