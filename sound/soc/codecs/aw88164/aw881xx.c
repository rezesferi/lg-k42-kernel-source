...
#ifdef AW88164_LGE_FACTORY
#include <linux/jiffies.h>
#include <linux/completion.h>
#endif

//bug 578896, duanzengchao@wingtech.com, 20200904, add mmitest and smartpa info
...
#define AW881XX_DRIVER_VERSION "v0.1.14"
...
static int aw881xx_dsp_control = 1;
#ifdef AW88164_LGE_FACTORY
static int aw881xx_mode_switching = 0;
#endif
...
{
	aw_dev_dbg(aw881xx->dev, "%s: enter,  mute = %d\n", __func__, mute);
...
{
	aw_dev_dbg(aw881xx->dev, "%s: enter,  pwd = %d \n", __func__, pwd);
...
{
	aw_dev_dbg(aw881xx->dev, "%s: enter, enable = %d \n", __func__, dsp);
...
static void aw881xx_dsp_update_cali_re(struct aw881xx *aw881xx)
{
	if (aw881xx->cali_attr.cali_re != AW_ERRO_CALI_VALUE) {
		...
		aw_dev_info(aw881xx->dev, "%s: write(%d) re to dsp\n",
...
#ifdef AW88164_LGE_FACTORY
	aw881xx->dsp_cfg_len[aw881xx->scene_mode] = cont->size;
#else
...
#ifdef AW88164_LGE_FACTORY
	if(aw881xx_mode_switching) {
		aw881xx_mode_switching = 0;
		complete_all(&aw881xx->fw_loading_completion);
	}
#endif
}
...
#ifdef AW88164_LGE_FACTORY
	aw881xx->dsp_fw_len[aw881xx->scene_mode] = cont->size;
#else
...
#ifdef AW88164_LGE_FACTORY
static void aw881xx_cold_start_lge(struct aw881xx *aw881xx)
{
	int ret = -1, i = 0;
	uint16_t iis_check_max = 5;
	bool next_step = false;
	uint8_t current_scene = 0;
	struct aw881xx_container *aw881xx_dsp_fw = NULL;
	struct aw881xx_container *aw881xx_dsp_cfg = NULL;

	aw_dev_info(aw881xx->dev, "%s: enter\n", __func__);

	if((aw881xx->cali_attr.cali_re != AW_ERRO_CALI_VALUE) && (aw881xx->cali_attr.cali_status == AW88164_CAL_STATUS_DONE)) {
		aw_dev_info(aw881xx->dev, "%s: skip getting re value.   re = %u\n", __func__, aw881xx->cali_attr.cali_re);
	} else if (aw881xx->cali_attr.cali_status == AW88164_CAL_STATUS_FAIL){
		aw881xx->cali_attr.cali_re = AW_ERRO_CALI_VALUE;
		aw_dev_info(aw881xx->dev, "%s: re calibration is failed. set  re to 0\n", __func__);
	} else {
		aw881xx_get_cali_re(&aw881xx->cali_attr);
	}

	aw881xx_get_cfg_shift(aw881xx);
	current_scene = aw881xx->scene_mode;

	//--------------------------------------------------------------------------------
	/** Apply Reg Bin **/
	mutex_lock(&aw881xx->lock);
	aw881xx_reg_container_update(aw881xx, aw881xx->aw881xx_cfgs[current_scene][0]);
	...
	if(!next_step) {
		aw_dev_err(aw881xx->dev, "%s: error loading reg\n", __func__);
		goto cold_start_exit;
	} else {
		next_step = false;
	}
	...
	aw881xx_dsp_enable(aw881xx, false);
	aw881xx_memclk_select(aw881xx, AW881XX_MEMCLK_PLL);
	aw881xx_dsp_fw = kzalloc(aw881xx->aw881xx_cfgs[current_scene][1]->len + sizeof(int), GFP_KERNEL);
	if (!aw881xx_dsp_fw) {
		aw_dev_err(aw881xx->dev, "%s: error allocating memory for DSP FW\n", __func__);
		goto cold_start_exit;
	}
	aw881xx_dsp_fw->len = aw881xx->aw881xx_cfgs[current_scene][1]->len;
	memcpy(aw881xx_dsp_fw->data, aw881xx->aw881xx_cfgs[current_scene][1]->data, aw881xx_dsp_fw->len);
	aw881xx_dsp_container_update(aw881xx, aw881xx_dsp_fw, aw881xx_base_addr[aw881xx->cfg_num +1]);

	//--------------------------------------------------------------------------------//
	/** Apply DSP CFG Bin **/
	aw881xx_dsp_cfg = kzalloc(aw881xx->aw881xx_cfgs[current_scene][2]->len + sizeof(int), GFP_KERNEL);
	if (!aw881xx_dsp_cfg) {
		aw_dev_err(aw881xx->dev, "%s: error allocating memory for DSP CFG\n", __func__);
		goto cold_start_exit;
	}
	aw881xx_dsp_cfg->len = aw881xx->aw881xx_cfgs[current_scene][2]->len;
	memcpy(aw881xx_dsp_cfg->data, aw881xx->aw881xx_cfgs[current_scene][2]->data, aw881xx_dsp_cfg->len);
	aw881xx_dsp_container_update(aw881xx, aw881xx_dsp_cfg, aw881xx_base_addr[aw881xx->cfg_num + 2]);
	aw881xx_dsp_update_cali_re(aw881xx);
	aw881xx_dsp_set_vcalb(aw881xx);
	...
	if (ret < 0) {
		aw881xx->init = AW881XX_INIT_NG;
		aw881xx_run_mute(aw881xx, true);
		aw_dev_info(aw881xx->dev, "%s:  DSP check error\n", __func__);
	} else {
		aw_dev_info(aw881xx->dev, "%s: DSP check is OK\n", __func__);
		if(aw881xx_mode_switching) {
			aw881xx->init = AW881XX_INIT_OK;
			aw881xx_stop(aw881xx);
		} else {
...
cold_start_exit:
	...
	if(aw881xx_dsp_fw != NULL)
		kfree(aw881xx_dsp_fw);
	aw881xx_dsp_fw = NULL;
	if(aw881xx_dsp_cfg != NULL)
		kfree(aw881xx_dsp_cfg);
	aw881xx_dsp_cfg = NULL;
...
#ifdef AW88164_LGE_FACTORY
			if(aw881xx->spk_FW_loaded && aw881xx->voice_FW_loaded)
				aw881xx_cold_start_lge(aw881xx);
			else
#endif
...
#ifdef AW88164_LGE_FACTORY
static int aw881xx_get_re_for_cal(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.aw_snd_soc_kcontrol_codec(kcontrol);
	struct aw881xx *aw881xx =
...
#ifdef AW88164_LGE_FACTORY_DEBUG
	pr_info("%s, cali_re_mohm = %u\n", __func__, aw881xx->cali_attr.cali_re_mohm);
#endif
	if(aw881xx->cali_attr.cali_status == AW88164_CAL_STATUS_NOT_YET) {
		ucontrol->value.integer.value[0] = AW_ERRO_CALI_VALUE;
	} else {
		ucontrol->value.integer.value[0] = aw881xx->cali_attr.cali_re_mohm;
	}
	return 0;
}

static int aw881xx_set_re_for_cal(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int aw881xx_get_f0_for_cal(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.aw_snd_soc_kcontrol_codec(kcontrol);
	struct aw881xx *aw881xx =
...
#ifdef AW88164_LGE_FACTORY_DEBUG
	pr_info("%s, f0 = %u\n", __func__, aw881xx->cali_attr.f0);
#endif

	ucontrol->value.integer.value[0] = aw881xx->cali_attr.f0;
	return 0;
}

static int aw881xx_set_f0_for_cal(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.aw_snd_soc_kcontrol_codec(kcontrol);
	struct aw881xx *aw881xx =
	...
	aw881xx->cali_attr.f0 = ucontrol->value.integer.value[0];
#ifdef AW88164_LGE_FACTORY_DEBUG
	pr_info("%s, f0= %u\n", __func__, aw881xx->cali_attr.f0);
#endif
	return 0;
...
static const char *const dsp_function[] = {
    "Off",
    "On",
};
...
#ifdef AW88164_LGE_FACTORY
static int wait_fw_time_ms = 210;
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0] == aw881xx->scene_mode && aw881xx->init == AW881XX_INIT_OK)
		return 1;

	aw881xx->scene_mode = ucontrol->value.integer.value[0];

	if (aw881xx->scene_mode == AW881XX_OFF_MODE) {
		return 0;
	}
	...
	if(!aw881xx->spk_FW_loaded || !aw881xx->voice_FW_loaded) {
		reinit_completion(&aw881xx->fw_loading_completion);

		aw_dev_dbg(aw881xx->dev, "%s: wait  for  f/w  loading....\n", __func__);

		wait_time = msecs_to_jiffies(wait_fw_time_ms);
		result = wait_for_completion_timeout(&aw881xx->fw_loading_completion, wait_time);
		if (result == 0) {
			aw_dev_err(aw881xx->dev, "%s:  f/w  loading  timeout.... \n", __func__);
			aw881xx_run_mute(aw881xx, true);
			aw881xx_run_pwd(aw881xx, true);
		} else {
			aw_dev_dbg(aw881xx->dev, "%s:  f/w  loading  is  completed.\n", __func__);
		}
	}
...
static int aw881xx_dsp_mode_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s: aw881xx_dsp_control = %d\n",
...
static int aw881xx_dsp_mode_set(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.aw_snd_soc_kcontrol_codec(kcontrol);
...
#ifdef AW88164_LGE_FACTORY
static const char * const calibration_status_text[] = {"false", "true", "running", "fail"};
static const char * const fw_loading_status_text[] = {"start","pass", "fail"};

static const struct soc_enum status_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(calibration_status_text), calibration_status_text),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(fw_loading_status_text), fw_loading_status_text),
};

static int aw881xx_get_cal_status(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.aw_snd_soc_kcontrol_codec(kcontrol);
...
#ifdef AW88164_LGE_FACTORY_DEBUG
	pr_info("%s, cal status = %u\n", __func__, aw881xx->cali_attr.cali_status);
#endif
	ucontrol->value.integer.value[0] = aw881xx->cali_attr.cali_status;
	return 0;
}

static int aw881xx_set_cal_status(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.aw_snd_soc_kcontrol_codec(kcontrol);
	...
	pr_info("%s, set value = %ld\n", __func__, ucontrol->value.integer.value[0]);

	if(ucontrol->value.integer.value[0] == AW88164_CAL_STATUS_NOT_YET) {
		if(aw881xx->cali_attr.cali_status == AW88164_CAL_STATUS_RUNNING) {
			pr_info("%s, don`t change cal status, calibration is running\n", __func__);
		} else {
			aw881xx->cali_attr.cali_re_mohm = 0;
			aw881xx->cali_attr.cali_status = AW88164_CAL_STATUS_NOT_YET;
		}
	} else if(ucontrol->value.integer.value[0] == AW88164_CAL_STATUS_FAIL) {
		aw881xx->cali_attr.cali_status = AW88164_CAL_STATUS_FAIL;
	}

	return 0;
}

static int aw881xx_get_f0_cal_status(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.aw_snd_soc_kcontrol_codec(kcontrol);
...
#ifdef AW88164_LGE_FACTORY_DEBUG
	pr_info("%s, f0 cal status = %u\n", __func__, aw881xx->cali_attr.f0_cali_status);
#endif
	ucontrol->value.integer.value[0] = aw881xx->cali_attr.f0_cali_status;
	return 0;
}

static int aw881xx_set_f0_cal_status(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.aw_snd_soc_kcontrol_codec(kcontrol);
	...
	aw881xx->cali_attr.f0_cali_status = ucontrol->value.integer.value[0];

#ifdef AW88164_LGE_FACTORY_DEBUG
	pr_info("%s, f0 cal status = %u, f0 = %u\n", __func__, aw881xx->cali_attr.f0_cali_status, aw881xx->cali_attr.f0);
#endif

	return 0;
}

static int aw881xx_get_fw_wait_ms(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = wait_fw_time_ms;
	return 0;
}

static int aw881xx_set_fw_wait_ms(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	wait_fw_time_ms = ucontrol->value.integer.value[0];
	return 0;
}

static int aw881xx_get_fw_loading_status(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.aw_snd_soc_kcontrol_codec(kcontrol);
...
static void aw881xx_early_load_fw(struct aw881xx *aw881xx);
static int aw881xx_set_fw_loading_status(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.aw_snd_soc_kcontrol_codec(kcontrol);
	...
	if(ucontrol->value.integer.value[0] == 2) {
		aw881xx->spk_FW_loaded = false;
		aw881xx->voice_FW_loaded = false;
		return 0;
	}

	if((!aw881xx->spk_FW_loaded || !aw881xx->voice_FW_loaded) && ucontrol->value.integer.value[0] == 0) {
		aw881xx_early_load_fw(aw881xx);
	}

	return 0;
}
#endif //AW88164_LGE_FACTORY
...
static struct snd_kcontrol_new aw881xx_controls[] = {
	...
	SOC_ENUM_EXT("aw881xx_dsp_switch",  aw881xx_snd_enum[1],
            aw881xx_dsp_mode_get, aw881xx_dsp_mode_set),
#ifdef AW88164_LGE_FACTORY
	SOC_SINGLE_EXT("aw881xx_re_val", SND_SOC_NOPM, 0, 0x4E20, 0, aw881xx_get_re_for_cal, aw881xx_set_re_for_cal),
	SOC_SINGLE_EXT("aw881xx_f0_val", SND_SOC_NOPM, 0, 0x4E20, 0, aw881xx_get_f0_for_cal, aw881xx_set_f0_for_cal),
	SOC_SINGLE_EXT("aw881xx_fw_wait_ms", SND_SOC_NOPM, 0, 0x03E8, 0, aw881xx_get_fw_wait_ms, aw881xx_set_fw_wait_ms),
	SOC_ENUM_EXT("aw881xx_calibration_status", status_enum[0], aw881xx_get_cal_status, aw881xx_set_cal_status),
	SOC_ENUM_EXT("aw881xx_f0_calibration_status", status_enum[0], aw881xx_get_f0_cal_status, aw881xx_set_f0_cal_status),
	SOC_ENUM_EXT("aw881xx_fw_loading_status", status_enum[1], aw881xx_get_fw_loading_status, aw881xx_set_fw_loading_status),
#endif
};
...
#ifdef AW88164_LGE_FACTORY
static void aw881xx_early_dsp_cfg_loaded_voice(const struct firmware *cont, void *context)
{
	struct aw881xx *aw881xx = context;
	uint16_t cfg_num = AW881XX_VOICE_MODE * AW881XX_MODE_CFG_NUM_MAX + 2;

	if (!cont) {
		aw_dev_err(aw881xx->dev, "%s: failed to read %s\n",
				__func__, aw881xx->cfg_name[cfg_num]);
		release_firmware(cont);
		return;
	}
	...
	aw_dev_info(aw881xx->dev,
			"%s: loaded %s - size: %zu\n", __func__,
			aw881xx->cfg_name[cfg_num], cont ? cont->size : 0);

	aw881xx->aw881xx_cfgs[AW881XX_VOICE_MODE][2] = kzalloc(cont->size + sizeof(int), GFP_KERNEL);
	if (!aw881xx->aw881xx_cfgs[AW881XX_VOICE_MODE][2]) {
		release_firmware(cont);
		aw_dev_err(aw881xx->dev, "%s: error allocating memory\n",
			   __func__);
		return;
	}
	aw881xx->dsp_cfg_len[AW881XX_VOICE_MODE] = cont->size;
	aw881xx->aw881xx_cfgs[AW881XX_VOICE_MODE][2]->len = cont->size;
	memcpy(aw881xx->aw881xx_cfgs[AW881XX_VOICE_MODE][2]->data, cont->data, cont->size);
	release_firmware(cont);

	aw881xx->voice_FW_loaded = true;
}

static void aw881xx_early_dsp_fw_loaded_voice(const struct firmware *cont, void *context)
{
	struct aw881xx *aw881xx = context;
	int ret = -1;
	uint16_t cfg_num = AW881XX_VOICE_MODE * AW881XX_MODE_CFG_NUM_MAX + 1;
	...
	aw881xx->aw881xx_cfgs[AW881XX_VOICE_MODE][1] = kzalloc(cont->size + sizeof(int), GFP_KERNEL);
	if (!aw881xx->aw881xx_cfgs[AW881XX_VOICE_MODE][1]) {
		release_firmware(cont);
		aw_dev_err(aw881xx->dev, "%s: error allocating memory\n",
				__func__);
		return;
	}
	aw881xx->dsp_fw_len[AW881XX_VOICE_MODE] = cont->size;
	aw881xx->aw881xx_cfgs[AW881XX_VOICE_MODE][1]->len = cont->size;
	memcpy(aw881xx->aw881xx_cfgs[AW881XX_VOICE_MODE][1]->data, cont->data, cont->size);
	release_firmware(cont);

	ret =  request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				aw881xx->cfg_name[cfg_num + 1],
				aw881xx->dev, GFP_KERNEL, aw881xx,
				aw881xx_early_dsp_cfg_loaded_voice);
	if (ret < 0) {
		aw_dev_err(aw881xx->dev, "%s: voice dsp cfg loading requested failed: %d\n", __func__, ret);
	}
}

static void aw881xx_early_reg_loaded_voice(const struct firmware *cont, void *context)
{
	struct aw881xx *aw881xx = context;
	int ret = -1;
	uint16_t cfg_num = AW881XX_VOICE_MODE * AW881XX_MODE_CFG_NUM_MAX;
	...
	aw881xx->aw881xx_cfgs[AW881XX_VOICE_MODE][0] = kzalloc(cont->size + sizeof(int), GFP_KERNEL);
	if (!aw881xx->aw881xx_cfgs[AW881XX_VOICE_MODE][0]) {
		release_firmware(cont);
		aw_dev_err(aw881xx->dev, "%s: error allocating memory\n",
				__func__);
		return;
	}
	aw881xx->aw881xx_cfgs[AW881XX_VOICE_MODE][0]->len = cont->size;
	memcpy(aw881xx->aw881xx_cfgs[AW881XX_VOICE_MODE][0]->data, cont->data, cont->size);
	release_firmware(cont);

	ret = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
					aw881xx->cfg_name[cfg_num + 1],
					aw881xx->dev, GFP_KERNEL, aw881xx,
					aw881xx_early_dsp_fw_loaded_voice);
	if (ret < 0) {
		aw_dev_err(aw881xx->dev, "%s: voice dsp fw loading requested failed: %d\n", __func__, ret);
	}
}

static void aw881xx_early_dsp_cfg_loaded_spk(const struct firmware *cont, void *context)
{
	struct aw881xx *aw881xx = context;
	uint16_t cfg_num = AW881XX_SPK_MODE * AW881XX_MODE_CFG_NUM_MAX + 2;
	...
	aw881xx->aw881xx_cfgs[AW881XX_SPK_MODE][2] = kzalloc(cont->size + sizeof(int), GFP_KERNEL);
	if (!aw881xx->aw881xx_cfgs[AW881XX_SPK_MODE][2]) {
		release_firmware(cont);
		aw_dev_err(aw881xx->dev, "%s: error allocating memory\n",
			   __func__);
		return;
	}
	aw881xx->dsp_cfg_len[AW881XX_SPK_MODE] = cont->size;
	aw881xx->aw881xx_cfgs[AW881XX_SPK_MODE][2]->len = cont->size;
	memcpy(aw881xx->aw881xx_cfgs[AW881XX_SPK_MODE][2]->data, cont->data, cont->size);
	release_firmware(cont);

	aw881xx->spk_FW_loaded = true;
}

static void aw881xx_early_dsp_fw_loaded_spk(const struct firmware *cont, void *context)
{
	struct aw881xx *aw881xx = context;
	int ret = -1;
	uint16_t cfg_num = AW881XX_SPK_MODE * AW881XX_MODE_CFG_NUM_MAX + 1;
	...
	aw881xx->aw881xx_cfgs[AW881XX_SPK_MODE][1] = kzalloc(cont->size + sizeof(int), GFP_KERNEL);
	if (!aw881xx->aw881xx_cfgs[AW881XX_SPK_MODE][1]) {
		release_firmware(cont);
		aw_dev_err(aw881xx->dev, "%s: error allocating memory\n",
				__func__);
		return;
	}
	aw881xx->dsp_fw_len[AW881XX_SPK_MODE] = cont->size;
	aw881xx->aw881xx_cfgs[AW881XX_SPK_MODE][1]->len = cont->size;
	memcpy(aw881xx->aw881xx_cfgs[AW881XX_SPK_MODE][1]->data, cont->data, cont->size);
	release_firmware(cont);

	ret =  request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				aw881xx->cfg_name[cfg_num + 1],
				aw881xx->dev, GFP_KERNEL, aw881xx,
				aw881xx_early_dsp_cfg_loaded_spk);
	if (ret < 0) {
		aw_dev_err(aw881xx->dev, "%s: spk dsp cfg loading requested failed: %d\n", __func__, ret);
	}
}

static void aw881xx_early_reg_loaded_spk(const struct firmware *cont, void *context)
{
	struct aw881xx *aw881xx = context;
	int ret = -1;
	uint16_t cfg_num = AW881XX_SPK_MODE * AW881XX_MODE_CFG_NUM_MAX;
	...
	aw881xx->aw881xx_cfgs[AW881XX_SPK_MODE][0] = kzalloc(cont->size + sizeof(int), GFP_KERNEL);
	if (!aw881xx->aw881xx_cfgs[AW881XX_SPK_MODE][0]) {
		release_firmware(cont);
		aw_dev_err(aw881xx->dev, "%s: error allocating memory\n",
				__func__);
		return;
	}
	aw881xx->aw881xx_cfgs[AW881XX_SPK_MODE][0]->len = cont->size;
	memcpy(aw881xx->aw881xx_cfgs[AW881XX_SPK_MODE][0]->data, cont->data, cont->size);
	release_firmware(cont);

	ret = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
					aw881xx->cfg_name[cfg_num + 1],
					aw881xx->dev, GFP_KERNEL, aw881xx,
					aw881xx_early_dsp_fw_loaded_spk);
	if (ret < 0) {
		aw_dev_err(aw881xx->dev, "%s: spk dsp fw loading requested failed: %d\n", __func__, ret);
	}
}
...
static void aw881xx_early_load_fw(struct aw881xx *aw881xx)
{
	...
	for(i = 0; i < AW881XX_CFG_MODE_NUM; i++) {
		for(j = 0; j < AW881XX_MODE_CFG_NUM_MAX; j++) {
			if(aw881xx->aw881xx_cfgs[i][j] != NULL) {
				kfree(aw881xx->aw881xx_cfgs[i][j]);
			}
			aw881xx->aw881xx_cfgs[i][j] = NULL;
		}
	}
	...
	ret =  request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				aw881xx->cfg_name[cfg_num],
				aw881xx->dev, GFP_KERNEL, aw881xx,
				aw881xx_early_reg_loaded_spk);

	if (ret < 0) {
		aw_dev_err(aw881xx->dev, "%s: spk cfg loading requested failed: %d\n", __func__, ret);
	}

	cfg_num = AW881XX_VOICE_MODE * AW881XX_MODE_CFG_NUM_MAX;
	ret = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				aw881xx->cfg_name[cfg_num],
				aw881xx->dev, GFP_KERNEL, aw881xx,
				aw881xx_early_reg_loaded_voice);

	if (ret < 0) {
		aw_dev_err(aw881xx->dev, "%s: voice cfg loading requested failed: %d\n", __func__, ret);
	}
...
#ifdef AW88164_LGE_FACTORY
	init_completion(&aw881xx->fw_loading_completion);
	for(i = 0; i < AW881XX_CFG_MODE_NUM; i++) {
		for(j = 0; j < AW881XX_MODE_CFG_NUM_MAX; j++) {
			aw881xx->aw881xx_cfgs[i][j] = NULL;
		}
	...
	if (aw881xx->dsp_cfg == AW881XX_DSP_BYPASS) {
		...
		if (ret < 0) {
			...
			aw881xx_i2c_write(aw881xx, AW881XX_REG_DSPMADD,
					AW881XX_SPK_DSP_FW_ADDR);
#ifdef AW88164_LGE_FACTORY
			for (i = 0; i < aw881xx->dsp_fw_len[aw881xx->scene_mode]; i += 2)
#else
			...
			aw881xx_i2c_write(aw881xx, AW881XX_REG_DSPMADD,
					AW881XX_SPK_DSP_CFG_ADDR);
#ifdef AW88164_LGE_FACTORY
			for (i = 0; i < aw881xx->dsp_cfg_len[aw881xx->scene_mode]; i += 2)
#else
			     ...
			     const struct i2c_device_id *id)
{
    ...
    //bug 578896, duanzengchao@wingtech.com, 20200904, add mmitest and smartpa info
    //if (aw881xx->chipid == 0x1806) {
    //    hardwareinfo_set_prop(HARDWARE_SMART_PA_ID, "AW88164");
    //} else {
    //    hardwareinfo_set_prop(HARDWARE_SMART_PA_ID, "unknown");
    //}
	if (ret < 0) {
