...
#define AWINIC_CALI_FILE  "/mnt/vendor/persist-lg/audio/aw_cali.bin"
...
static int aw881xx_get_cali_re_from_file(uint32_t *cali_re,
#ifdef AW88164_LGE_FACTORY
				uint32_t *cali_re_mohm,
#endif
				char *name_suffix)
{
	struct file *fp = NULL;
...
#ifdef AW88164_LGE_FACTORY
	return aw881xx_get_cali_re_from_file(&cali_attr->cali_re, &cali_attr->cali_re_mohm, aw881xx->name_suffix);
#else 
...
void aw881xx_get_cali_re(struct aw881xx_cali_attr *cali_attr)
{
	...
	if (ret < 0) {
#ifdef AW88164_LGE_FACTORY
		cali_attr->cali_status = AW88164_CAL_STATUS_FAIL;
#endif
...
#ifdef AW88164_LGE_FACTORY
	cali_attr->cali_status = AW88164_CAL_STATUS_DONE;
#ifdef AW88164_LGE_FACTORY_DEBUG
	pr_info("%s,  cali_re = %u,  dsp_ra = %u, total = %u, cali_status = %u\n", __func__, cali_attr->cali_re, dsp_ra, cali_re, cali_attr->cali_status);
#endif
#endif
				...
				size_t count)
{
	...
	cali_attr->cali_re = cali_attr->re;
#ifdef AW88164_LGE_FACTORY
	cali_attr->cali_re_mohm = cali_attr->cali_re;
	cali_attr->cali_status = AW88164_CAL_STATUS_RUNNING;
#ifdef AW88164_LGE_FACTORY_DEBUG
	pr_info("%s, cali_re =: %u, , cali_status = %u\n", __func__, cali_attr->cali_re, cali_attr->cali_status);
#endif
#endif
	return count;
}
...
void aw881xx_cali_init(struct aw881xx_cali_attr *cali_attr)
{
	...
	cali_attr->re = 0;
	cali_attr->f0 = 0;
	cali_attr->cali_re_mohm = 0;
	cali_attr->cali_status = AW88164_CAL_STATUS_NOT_YET;
	cali_attr->f0_cali_status = AW88164_CAL_STATUS_NOT_YET;
#endif
