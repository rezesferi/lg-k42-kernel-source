...
extern int rcv_boost;
extern int rcv_volume;
...
static int fs1603_write_preset_eq(fsm_dev_t *fsm_dev, int ram_id, uint16_t scene)
{
    ...
    //for npreset
    if (((!preset_list || i >= dev_list->len)) && (scene == FSM_SCENE_VOICE)) {
    ...
    } else if (!preset_list || i >= dev_list->len) {
...
{
    int diff_val, step_unit;
    uint8_t re25_byte = 0, offset;
...
{
    int ret, count_new, ready, re25_new;
    uint16_t otprdata;
...
{
    int ready, count, delta, re25_old, re25_new, ret;
    uint16_t pllctrl4, bstctrl, otprdata;
...
{
    int calib_min, calib_max, re25, ret;
    uint16_t zmdata, value;
...
//foursemi            volume = 0xdf;// 0xf8 = -2.625dB, 0xdf = -12db
// 190906 add LGE rcv mode register set S
            volume = rcv_volume;
            ret |= fsm_reg_write(fsm_dev, REG(BSTCTRL), rcv_boost);
// 190906 add LGE rcv mode register set E
        }
