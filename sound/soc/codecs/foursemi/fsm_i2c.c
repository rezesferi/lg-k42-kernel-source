...
int fsm_i2c_cmd(int cmd, int argv)
{
    ...
    list_for_each_entry(data_ptr, &fsm_device_list, list)
    {
        ...
        if(((cmd == FSM_CMD_CALIB_STEP1) || (cmd == FSM_CMD_CALIB_STEP2) || (cmd == FSM_CMD_CALIB_STEP3)) &&
            ((fsm_dev->dev_state & STATE(CALIB_DONE)) != 0) && !argv){
    ...
    if((cmd == FSM_CMD_CALIB_STEP1) && (flag & STATE(CALIB_DONE)) != 0)
...
int fsm_calibrate(int force)
{
    ...
    ret |= fsm_i2c_cmd(FSM_CMD_CALIB_STEP2, 0);
...
int fsm_get_r25_str(char *buf, int max_size)
{
    ...
    list_for_each_entry(data_ptr, &fsm_device_list, list) {
        ...
        if (fsm_dev->i2c->addr == FSM_DEV_LEFT) {
            if (ohms <= 8704 && ohms >= 6144)
                result = 1;
            else
                result = 0;
        } else if (fsm_dev->i2c->addr == FSM_DEV_RIGHT) {
            if (ohms <= 7680 && ohms >= 6144)
                result = 1;
            else
                result = 0;
        } else {
            pr_err("invalid device addr:0x%02x",fsm_dev->i2c->addr);
        }
        len = snprintf(buf + str_len, max_size, "%02x:%d %d result:%d",
                fsm_dev->i2c->addr, ohms, fsm_dev->errcode, result);
        str_len += len;
        mutex_unlock(&data_ptr->i2c_lock);
    }
    if (str_len > max_size) {
...
int fsm_get_registers(uint8_t this_i2c_addr, uint8_t reg)
{
    fsm_pdata_t *data_ptr = NULL;
    uint16_t value = 0;
    int ret = 0;
    ...
    list_for_each_entry(data_ptr, &fsm_device_list, list)
    {
        if (data_ptr->fsm_dev.i2c_addr == this_i2c_addr)
...
int fsm_set_registers(uint8_t this_i2c_addr, uint8_t reg, uint16_t value)
{
    int ret = 0;
    fsm_pdata_t *data_ptr = NULL;
    ...
    {
        if (data_ptr->fsm_dev.i2c_addr == this_i2c_addr)
            ret = fsm_reg_write(&data_ptr->fsm_dev, reg, value);
    }
    fsm_mutex_unlock();

    return ret;
}
// 190819 add LGE get/set registers E
// 191021 add LGE channel swap S
int fsm_channel_swap(int channel_swap_info)
{
    int ret = 0;
    fsm_pdata_t *data_ptr = NULL;
    ...
    {
        audioctrl.bit.VOL = 0;
        ret |= fsm_reg_write(&data_ptr->fsm_dev, FS1603_AUDIOCTRL, audioctrl.value);
        ret |= fsm_reg_read(&data_ptr->fsm_dev, FS1603_I2SCTRL, &i2sctrl.value);
        if (channel_swap_info) {
            if (data_ptr->fsm_dev.i2c_addr == FSM_DEV_LEFT)
                i2sctrl.bit.CHS12 = FSM_CHN_RIGHT;
            else if (data_ptr->fsm_dev.i2c_addr == FSM_DEV_RIGHT)
                i2sctrl.bit.CHS12 = FSM_CHN_LEFT;
        } else {
            if (data_ptr->fsm_dev.i2c_addr == FSM_DEV_LEFT)
                i2sctrl.bit.CHS12 = FSM_CHN_LEFT;
            else if (data_ptr->fsm_dev.i2c_addr == FSM_DEV_RIGHT)
        ...
        pr_info("%s CHS12: %d, val: 0x%04x\n", __func__, i2sctrl.bit.I2SSR, i2sctrl.value);
        ret |= fsm_reg_write(&data_ptr->fsm_dev, FS1603_I2SCTRL, i2sctrl.value);

        fsm_delay_ms(10);
        audioctrl.bit.VOL = 0xff;
        ret |= fsm_reg_write(&data_ptr->fsm_dev, FS1603_AUDIOCTRL, audioctrl.value);
    }
    fsm_mutex_unlock();
...
int fsm_get_calib_impedance(uint8_t this_i2c_addr)
{
    fsm_pdata_t *data_ptr = NULL;
    int return_value = 0;
    ...
    {
        if (data_ptr->fsm_dev.i2c_addr == this_i2c_addr)
            return_value = data_ptr->fsm_dev.calib_data.calib_re25;
    }
    fsm_mutex_unlock();
...
int fsm_get_calib_status(uint8_t this_i2c_addr)
{
    fsm_pdata_t *data_ptr = NULL;
    int return_value = 0;
    ...
    {
        if (data_ptr->fsm_dev.i2c_addr == this_i2c_addr)
            return_value = (data_ptr->fsm_dev.dev_state & STATE(CALIB_DONE));
    }
    fsm_mutex_unlock();
