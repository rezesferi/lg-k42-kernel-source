...
static int fsm_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
    ...
    {
        dev_err(&i2c->dev, "%s check_functionality failed\n", __func__);
        return -EIO;
    }
    ...
    for (i = 0; i < FSM_DEV_MAX; i++) {
        ...
        pr_err("%s, addr: 0x%x, device id :0x%x.\n", __func__, i2c->addr, dev_id);
        if (((dev_id & 0xFF00) >> 8) == FS1603_DEV_ID) {
            pr_err("%s, fs1894 detected.\n", __func__);
            ndev++;
        }
    }
    if (ndev == 0) {
        pr_err("%s, not foursemi device, return", __func__);
        return -EINVAL;
    }
#if 0
    ...
    i2c_set_clientdata(i2c, fsm_dev);
     ...
     if(ret != 0)
         pr_err("%s, misc register failed, ret: %d.", __func__, ret);

    pr_err("%s, i2c probe completed", __func__);
