...
#ifdef CONFIG_LGE_ACCDET_SWITCH
...
static u32 button_press_debounce = 0x210;
#else
static u32 button_press_debounce = 0x400;
#endif

#if defined(CONFIG_LGE_ACCDET) && !defined(CONFIG_WORKAROUND_OPEN_CABLE)
static bool auxCableInt = false;
#endif

#ifdef CONFIG_LGE_ACCDET_SWITCH
static struct switch_dev accdet_switch;
#endif
...
static void send_status_event(u32 cable_type, u32 status)
{
	...
	switch (cable_type) {
	case HEADSET_NO_MIC:
#if 0 //Comment out unnecessary code.
//#ifdef CONFIG_LGE_ACCDET_SWITCH
		if(HEADSET_MIC == accdet_switch.state && status == 1){
			snd_soc_jack_report(&accdet->jack, 0, SND_JACK_MICROPHONE);
			snd_soc_jack_report(&accdet->jack, 0, SND_JACK_HEADPHONE);
		}
#endif
		if (status)
		 ...
		 * visible.
		 */
#ifndef CONFIG_LGE_ACCDET_SWITCH
		if (status == 0) {
			report = 0;
			snd_soc_jack_report(&accdet->jack, report,
					SND_JACK_MICROPHONE);
...
#if 0 //Comment out unnecessary code.
//#ifdef CONFIG_LGE_ACCDET_SWITCH
		if(HEADSET_NO_MIC == accdet_switch.state && status == 1){
			snd_soc_jack_report(&accdet->jack, 0, SND_JACK_HEADPHONE);
		}
		snd_soc_jack_report(&accdet->jack, status == 0?0:SND_JACK_HEADPHONE, SND_JACK_HEADPHONE);
		snd_soc_jack_report(&accdet->jack, status == 0?0:SND_JACK_MICROPHONE, SND_JACK_MICROPHONE);
#else //MTK Original
		if (status == 0) {
			report = 0;
			snd_soc_jack_report(&accdet->jack, report,
	...
	case LINE_OUT_DEVICE:
		...
		pr_info("accdet LineOut %s\n",
			status ? "PlugIn" : "PlugOut");
		break;
#if defined(CONFIG_LGE_ACCDET_SWITCH) && defined(CONFIG_LGE_ACCDET_MOISTURE) //OS LGE Feature with defined(CONFIG_MOISTURE_INT_SUPPORT) || defined(CONFIG_MOISTURE_EXT_SUPPORT)
	case MOISTURE:
		snd_soc_jack_report(&accdet->jack, status == 0?0:MOISTURE_DEVICE, MOISTURE_DEVICE);
...
#ifdef CONFIG_LGE_ACCDET_SWITCH
	if(status)
		switch_set_state((struct switch_dev *)&accdet_switch, cable_type);
	else
		switch_set_state((struct switch_dev *)&accdet_switch, NO_DEVICE);
#endif
}
...
#if defined(CONFIG_LGE_ACCDET) && !defined(CONFIG_WORKAROUND_OPEN_CABLE)
	// Change ACCDET input to ACCDET pin if mic mode is acc.  need to check if this is necessary.
	if(auxCableInt && (accdet_dts.mic_mode == HEADSET_MODE_1)){
		auxCableInt = false;
		accdet_write(MT6357_AUDENC_ANA_CON10, accdet_read(MT6357_AUDENC_ANA_CON10) ^ RG_ACCDET_MODE_ANA10_AUXINT);
	}
#endif
}
...
#if defined(CONFIG_LGE_ACCDET) && !defined(CONFIG_WORKAROUND_OPEN_CABLE)
		// Change ACCDET input to MICP pin. need to check if this is necessary.
		if((ACCDET_STATE_AB_MASK & (accdet_read(ACCDET_MEM_IN_ADDR) >> ACCDET_STATE_MEM_IN_OFFSET)) == ACCDET_STATE_AB_11 
...
#ifdef CONFIG_WORKAROUND_OPEN_CABLE
	pr_info("accdet %s(): cur_AB = %u, force_4pole = %d, accdet_status = %u\n", __func__, cur_AB, force_4pole, accdet->accdet_status);
	...
	case PLUG_OUT:
		if (cur_AB == ACCDET_STATE_AB_00) {
			...
			if (HAS_CAP(accdet->data->caps,
				...
				mutex_lock(&accdet->res_lock);
#ifdef CONFIG_WORKAROUND_OPEN_CABLE
				if (accdet->eint_sync_flag & force_4pole) {
					accdet->accdet_status = MIC_BIAS;
					accdet->cable_type = HEADSET_MIC;
				} else if (accdet->eint_sync_flag) {
					accdet->accdet_status = PLUG_OUT;
					accdet->cable_type = NO_DEVICE;
				} else
...
#ifdef CONFIG_LGE_ACCDET
			send_status_event(pre_cable_type, 0);
#endif
			send_status_event(accdet->cable_type, 1);
	}
	mutex_unlock(&accdet->res_lock);
		...
		if (accdet->water_r != 0) {
			if (accdet->cur_eint_state ==
				...
				pr_info("%s Moisture plug out detectecd\n",
					__func__);
#ifdef CONFIG_LGE_ACCDET_MOISTURE //OS LGE Feature with defined(CONFIG_MOISTURE_INT_SUPPORT) || defined(CONFIG_MOISTURE_EXT_SUPPORT)
				send_status_event(MOISTURE, 0);
#endif
...
#ifdef CONFIG_LGE_ACCDET //QOS LGE Feature with ANALOG_FASTDISCHARGE_SUPPORT
	// Set EINT Comparator TH. to 2V (Default 2.4v)... need to check if this is necessary.
	accdet_write(MT6357_AUDENC_ANA_CON10, accdet_read(MT6357_AUDENC_ANA_CON10) | RG_ACCDET_MODE_ANA10_FASTDISCHARGER);
#endif
...
#ifdef CONFIG_LGE_ACCDET_SWITCH
	accdet_switch.name = "h2w";
	accdet_switch.index = 0;
	accdet_switch.state = NO_DEVICE;
	ret = switch_dev_register(&accdet_switch);
	if (ret) {
...
static int accdet_remove(struct platform_device *pdev)
{
	...
	class_destroy(accdet->accdet_class);
#ifdef CONFIG_LGE_ACCDET_SWITCH
	switch_dev_unregister(&accdet_switch);
#endif
