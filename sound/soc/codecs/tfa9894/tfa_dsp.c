...
int tfa_irq_get(struct tfa_device *tfa, enum tfa9912_irq bit)
{
	...
	if (bit < tfa9912_irq_max) {
		...
		mask = 1<<(bit & 0x0f);
		tfa_smartpa_reg_read(tfa, (unsigned char)reg, &value);
	} else
		return -1;
...
int tfa98xx_powerswitch_is_enabled(struct tfa_device *tfa)
{
	...
	if (((tfa->rev & 0xff) == 0x13) || ((tfa->rev & 0xff) == 0x88)) {
		ret = tfa_smartpa_reg_read(tfa, 0xc6, &value);
...
void tfa2_manual_mtp_cpy(struct tfa_device *tfa,  uint16_t reg_row_to_keep,uint16_t reg_row_to_set ,uint8_t row)///MCH_TO_TEST
{
	...
	/* Assure FAIM is enabled (enable it when neccesery) */
		...
		tfa_smartpa_reg_read(tfa, (unsigned char)reg_row_to_keep, &value);
		if(!row)
		{
...
enum Tfa98xx_Error tfa_smartpa_reg_read(struct tfa_device *tfa, unsigned char subaddress, unsigned short *value)
{
	enum Tfa98xx_Error error;
...
int tfa_set_bf(struct tfa_device *tfa, const uint16_t bf, const uint16_t value)
{
	...
	err = tfa_smartpa_reg_read(tfa, address, &regvalue);
	if (err) {
...
enum Tfa98xx_Error show_current_state(struct tfa_device *tfa)
{
	...
	if (tfa->tfa_family == 2 && tfa->verbose) {
		...
		printk("Current HW manager state: ");

		switch(manstate) {
			case 0: printk("power_down_state \n");
				break;
			case 1: printk("wait_for_source_settings_state \n");
				break;
			case 2: printk("connnect_pll_input_state \n");
				break;
			case 3: printk("disconnect_pll_input_state \n");
				break;
			case 4: printk("enable_pll_state \n");
				break;
			case 5: printk("enable_cgu_state \n");
				break;
			case 6: printk("init_cf_state \n");
				break;
			case 7: printk("enable_amplifier_state \n");
				break;
			case 8: printk("alarm_state \n");
				break;
			case 9: printk("operating_state \n");
				break;
			case 10: printk("mute_audio_state \n");
				break;
			case 11: printk("disable_cgu_pll_state \n");
				break;
			default:
				printk("Unable to find current state \n");
				break;
		}
	}
...
#define MANSTATE_RETRY_TIMES (20)
enum tfa_error tfa_dev_stop(struct tfa_device *tfa)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	int manstate = 0, retry = 0;

	if(1 == TFA_GET_BF(tfa, PWDN)) {
		goto error_exit;
	}
	/* mute */
	tfaRunMute(tfa);
	...
	manstate = TFA_GET_BF(tfa, MANSTATE);
	while((manstate >= 4) && (retry < MANSTATE_RETRY_TIMES)){
		msleep_interruptible(10);
		manstate = TFA_GET_BF(tfa, MANSTATE);
		retry++;
	}

	if(retry >= MANSTATE_RETRY_TIMES)
		pr_info("%s state machine to powerdown.MANSTATE %d over %d times\n",
			__func__, manstate, MANSTATE_RETRY_TIMES);
error_exit:
	if(err != Tfa98xx_Error_Ok) {
		pr_err("FA98xx Error code is 0x%04x", err);
	}
	return (enum tfa_error)err;
}
	...
	switch (item) {
		...
		case TFA_MTP_RE25_PRIM:
			if(tfa->tfa_family == 2) {
				...
				tfa_smartpa_reg_read(tfa, 0x83, (unsigned short*)&value);
			}
			break;
