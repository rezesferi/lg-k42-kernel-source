...
static enum Tfa98xx_Error tfa9912_specific(struct tfa_device *tfa)
{
	...
	error = tfa_smartpa_reg_read(tfa, 0xFB, &value);
	xor = value ^ 0x005A;
...
static uint16_t tfa9912_vboost_fixup(struct tfa_device *tfa, uint16_t dcdc_cnt6)
{
	...
	tfa_smartpa_reg_read(tfa, TFA9912_CALIBR_REG, &cal_offset);
...
static enum Tfa98xx_Error tfa9872_specific(struct tfa_device *tfa)
{
	...
	error = tfa_smartpa_reg_read(tfa, 0xB0, &value);
	value |= 1 << 3;
...
static enum Tfa98xx_Error tfa9896_specific(struct tfa_device *tfa)
{
	...
	*/
	...
	error = tfa_smartpa_reg_read(tfa, 0x49, &check_value);
	check_value &= ~0x1;
...
static enum Tfa98xx_Error tfa9890_specific(struct tfa_device *tfa)
{
	...
	if (error)
		return error;
	tfa_smartpa_reg_read(tfa, 0x59, &regRead);
	regRead |= 0x3;
...
static enum Tfa98xx_Error tfa9890_clockgating(struct tfa_device *tfa, int on)
{
	...
	error = tfa_smartpa_reg_read(tfa, TFA98XX_CURRENTSENSE4, &value);
...
static enum Tfa98xx_Error tfa9890_dsp_system_stable(struct tfa_device *tfa, int *ready)
{
	...
	*ready = !((TFA_GET_BF_VALUE(tfa, AREFS, status) == 0)
	...
	error = tfa_smartpa_reg_read(tfa, 0x84, &mtp0);
	if (error)
		goto errorExit;
...
static enum Tfa98xx_Error tfa9894_set_default_calib(struct tfa_device *tfa, int re25)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	...
	// enable faimvbgovrrl
    tfa_set_bf(tfa, TFA9894_BF_FAIMVBGOVRRL, 1);

	//hardcode calibration value 8 (0x2000):
    tfa2_manual_mtp_cpy(tfa, 0xF4, (unsigned short)re25, 2);

	//hardcode both MTPEX and MTPOTC to 1:
    tfa_smartpa_reg_read(tfa, 0xF0, &value);
    value |= 0x03;
    tfa2_manual_mtp_cpy(tfa, 0xF1, value, 0);
...
void tfa9894_ops(struct tfa_device_ops *ops)
{
	...
	ops->set_default_calib = tfa9894_set_default_calib;
}
