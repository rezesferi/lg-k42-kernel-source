...
static int tas2563_file_release(struct inode *inode, struct file *file)
{
	struct tas2563_priv *pTAS2563 =
			(struct tas2563_priv *)file->private_data;
...
static ssize_t tas2563_file_read(struct file *file, char *buf,
			size_t count, loff_t *ppos)
{
	...
	case TIAUDIO_CMD_REG_READ: {
		...
		if (count == 1) {
			ret = pTAS2563->read(pTAS2563,
			pTAS2563->mnCurrentChannel, pTAS2563->mnCurrentReg,
			&nValue);
			if (ret < 0)
				break;
			...
			if (p_kBuf != NULL) {
				ret = pTAS2563->bulk_read(pTAS2563,
...
static ssize_t tas2563_file_write(struct file *file, const char *buf,
				size_t count, loff_t *ppos)
{
	struct tas2563_priv *pTAS2563 =
			(struct tas2563_priv *)file->private_data;
	...
	case TIAUDIO_CMD_REG_WITE:
		if (count > 6) {
			...
			if (len == 1) {
				...
				ret = pTAS2563->bulk_write(pTAS2563, chl,
					reg, &p_kBuf[6], len);
		} else
			dev_err(pTAS2563->dev,
	...
	case TIAUDIO_CMD_REG_READ:
		if (count == 6) {
			pTAS2563->mnCurrentChannel = p_kBuf[1];
			pTAS2563->mnCurrentReg =
