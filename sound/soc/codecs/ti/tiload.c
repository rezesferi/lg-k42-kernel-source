...
static int tiload_release(struct inode *in, struct file *filp)
{
	struct tas2563_priv *pTAS2563 =
			(struct tas2563_priv *)filp->private_data;
	...
	size_t count, loff_t *offset)
{
	...
	//dev_dbg(pTAS2563->dev,
		//"nCompositeRegister:0x%x gBook:0x%x gpage:0x%x\n",
		//nCompositeRegister, gBook, gPage);
	if (count == 1) {
		ret =
			pTAS2563->read(pTAS2563, pTAS2563->mnCurrentChannel,
				0x80000000 | nCompositeRegister, &Value);
		if (ret >= 0)
			rd_data[0] = (char) Value;
	} else if (count > 1) {
		ret =
			pTAS2563->bulk_read(pTAS2563,
 ...
 *		dev_err(pTAS2563->dev, "rd_data[%d]=%x\n", i, rd_data[i]);
 *
 * #endif
 */
	if (size != count)
 ...
 *		dev_err(pTAS2563->dev, "rd_data[%d]=%x\n", i, rd_data[i]);
 *
 *	dev_info(pTAS2563->dev, "channel: %x\n", pTAS2563->mnCurrentChannel);
 */
 ...
 */
	...
	if (count == 2) {
		ret =
			pTAS2563->write(pTAS2563, pTAS2563->mnCurrentChannel,
					0x80000000 | nCompositeRegister,
			pData[1]);
	} else if (count > 2) {
		ret =
			pTAS2563->bulk_write(pTAS2563,
					pTAS2563->mnCurrentChannel,
					0x80000000 | nCompositeRegister,
			&pData[1], count - 1);
	}
...
static long tiload_ioctl(struct file *filp, unsigned int cmd,
						unsigned long arg)
{
	struct tas2563_priv *pTAS2563 =
				(struct tas2563_priv *)filp->private_data;
 ...
 */
	...
	case TILOAD_IOCTL_SET_CHL:
		...
		if (addr == pTAS2563->mnRAddr) {
			dev_info(pTAS2563->dev,
				"TILOAD_IOCTL_SET_CHL right=0x%x\n", cmd);
			pTAS2563->mnCurrentChannel = channel_right;
		} else if (addr == pTAS2563->mnLAddr) {
			dev_info(pTAS2563->dev,
				"TILOAD_IOCTL_SET_CHL left=0x%x\n", cmd);
			pTAS2563->mnCurrentChannel = channel_left;
		} else {
			dev_err(pTAS2563->dev,
			"TILOAD_IOCTL_SET_CHL: err l:%x r:%x cmd:%x addr :%x\n\r",
			pTAS2563->mnLAddr, pTAS2563->mnRAddr, cmd, addr);
		}
		break;
	case TILOAD_IOCTL_SET_CONFIG:
		dev_info(pTAS2563->dev,
			"%s, cmd=TILOAD_IOCTL_SET_CONFIG\n", __func__);
		num = copy_from_user(&val, argp, sizeof(val));
		//pTAS2563->set_config(pTAS2563, val);
		pTAS2563->set_config(pTAS2563, channel_left, val);
		pTAS2563->set_config(pTAS2563, channel_right, val);
		break;
	case TILOAD_IOCTL_SET_CALIBRATION:
		...
		pTAS2563->set_calibration(pTAS2563, channel_left, val);
		pTAS2563->set_calibration(pTAS2563, channel_right, val);
		break;

	default:
		dev_info(pTAS2563->dev, "TILOAD unsupport cmd %x\n", cmd);
		break;
	}
	return num;
}
...
int tiload_driver_init(struct tas2563_priv *pTAS2563)
{
	...
	if (device_create(tiload_class, NULL,
			dev, NULL, "tiload_node") == NULL)
