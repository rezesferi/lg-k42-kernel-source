...
//Added by LGE for Stereo SPK_HP model
...
#ifndef CONFIG_LGE_SKIP_SPK_DCTRIM_CALIB
...
#endif //#ifndef CONFIG_LGE_SKIP_SPK_DCTRIM_CALIB
#endif
...
static void set_lr_trim_code(void)
{
	...
	hpr_base = hpr_dc_offset;
EXIT:
#ifdef CONFIG_LGE_HP_DC_OFFSET_LIMIT
	/* Step4: check DC compensation < 1.5mV to avoid noise flow > 5uV*/
	/* Check L hp trimcode 0xA : 1.5mV, finetrim 0x2: +0.5mV*/
	if ((trimcode[AUDIO_ANALOG_CHANNELS_LEFT1] > 0xa)||
		(trimcode[AUDIO_ANALOG_CHANNELS_LEFT1] == 0xa &&
		finetrim[AUDIO_ANALOG_CHANNELS_LEFT1] > 0x1)) {
		trimcode[AUDIO_ANALOG_CHANNELS_LEFT1] = 0xa;
		finetrim[AUDIO_ANALOG_CHANNELS_LEFT1] = 0x1;
	}
	/* Check R */
	if ((trimcode[AUDIO_ANALOG_CHANNELS_RIGHT1] > 0xa)||
		(trimcode[AUDIO_ANALOG_CHANNELS_RIGHT1] == 0xa &&
...
#ifdef CONFIG_LGE_HP_DC_OFFSET_LIMIT
	/* Step4: check DC compensation < 1.5mV to avoid noise flow > 5uV*/
	/* Check L skp trimcode 0xA : 1.5mV, finetrim 0x2: +0.5mA 0x3: > 0mV*/
	if ((trimcode[AUDIO_ANALOG_CHANNELS_LEFT1] > 0xa) ||
		(trimcode[AUDIO_ANALOG_CHANNELS_LEFT1] == 0xa &&
		finetrim[AUDIO_ANALOG_CHANNELS_LEFT1] > 0x1)) {
		trimcode[AUDIO_ANALOG_CHANNELS_LEFT1] = 0xa;
...
static void TurnOnDacPower(int device)
{
	...
	//Modified by LGE for Stereo SPK_HP model : Bit 10 must not be changed because of added code related with AUDIO_ANALOG_DEVICE_OUT_RECEIVER_HEADSET_L
	//Ana_Set_Reg(AFUNC_AUD_CON0, 0xCBA1, 0xffff); //MTK Original
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	//Modified by LGE for Stereo SPK_HP model
	if (mCodec_data->mAudio_Ana_DevicePower
		[AUDIO_ANALOG_DEVICE_OUT_HEADSETL] ||
		mCodec_data->mAudio_Ana_DevicePower
		[AUDIO_ANALOG_DEVICE_OUT_SPEAKER_HEADSET_R] ||
