...
enum audio_system_gpio_type {
	...
	GPIO_RECEIVER_EXAMP_HIGH,
	GPIO_RECEIVER_EXAMP_LOW,
#endif
	GPIO_NUM
};
...
static struct audio_gpio_attr aud_gpios[GPIO_NUM] = {
		...
		[GPIO_AUD_CLK_MOSI_LOW] = {"aud_clk_mosi_pull_low", false,
					   NULL},
#ifdef CONFIG_RECEIVER_EXTAMP_USE
		[GPIO_RECEIVER_EXAMP_HIGH] = {"receiver-extamp-pullhigh", false, NULL},
		[GPIO_RECEIVER_EXAMP_LOW] = {"receiver-extamp-pulllow", false, NULL},
#endif
};
...
#ifdef CONFIG_RECEIVER_EXTAMP_USE
extern bool mAudio_HAC_Switch;
int AudDrv_GPIO_RCV_EXTAMP_Select(int bEnable, int mode)
{
	int retval = 0;
	int i;
	pr_info("Select GPIO of RCV EXTAMP : %d\n", bEnable);
	mutex_lock(&gpio_request_mutex);
	if (bEnable == 1) {
		mAudio_HAC_Switch = true;
		if (aud_gpios[GPIO_RECEIVER_EXAMP_HIGH].gpio_prepare) {
			for(i = 0; i < mode; i++) {
				retval = pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_RECEIVER_EXAMP_LOW].gpioctrl);
				if (retval)
					pr_info("could not set aud_gpios[GPIO_RECEIVER_EXAMP_LOW pins\n");
				udelay(2);
				retval = pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_RECEIVER_EXAMP_HIGH].gpioctrl);
				if (retval)
					pr_info("could not set aud_gpios[GPIO_RECEIVER_EXAMP_HIGH] pins\n");
				udelay(2);
			}
		}
	} else {
		mAudio_HAC_Switch = false;
		if (aud_gpios[GPIO_RECEIVER_EXAMP_LOW].gpio_prepare) {
			retval = pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_RECEIVER_EXAMP_LOW].gpioctrl);
			if (retval)
				pr_info("could not set aud_gpios[GPIO_RECEIVER_EXAMP_LOW pins\n");
		}
